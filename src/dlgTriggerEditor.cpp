...
dlgTriggerEditor::dlgTriggerEditor(Host *pH)
    : mpAliasBaseItem(nullptr), mpTriggerBaseItem(nullptr), mpScriptsBaseItem(nullptr), mpTimerBaseItem(nullptr),
      mpActionBaseItem(nullptr), mpKeyBaseItem(nullptr), mpVarBaseItem(nullptr), mpCurrentActionItem(nullptr),
      mpCurrentKeyItem(nullptr), mpCurrentTimerItem(nullptr), mpCurrentScriptItem(nullptr),
      mpCurrentTriggerItem(nullptr), mpCurrentAliasItem(nullptr), mpCurrentVarItem(nullptr), mpHost(pH),
      mpSourceEditorDocument(nullptr), mpSourceEditorEdbee(nullptr), mpSourceEditorEdbeeDocument(nullptr),
      mSearchOptions(pH->mSearchOptions), mpAction_searchOptions(nullptr), mIcon_searchOptions(QIcon()),
      mpAction_searchCaseSensitive(nullptr), mpAction_searchIncludeVariables(nullptr)
      // TODO: Implement other searchOptions:
      //, mpAction_searchWholeWords(nullptr)
      //, mpAction_searchRegExp(nullptr)
      ,
      mCleanResetQueued(false), mSavingAs(false), mAutosaveInterval{}, mTriggerEditorSplitterState{},
      mAliasEditorSplitterState{}, mScriptEditorSplitterState{}, mActionEditorSplitterState{},
      mKeyEditorSplitterState{}, mTimerEditorSplitterState{}, mVarEditorSplitterState{}
{
    // init generated dialog
    setupUi(this);

    msgInfoAddAlias =
        tr("<p>Alias react on user input. To add a new alias:"
           ...
           "<li>Define a 'substitution' <strong>command</strong> to send to the game in clear text <strong>instead of "
           "the alias pattern</strong>, or write a script for more complicated needs.</li>"
           "<li><strong>Activate</strong> the alias.</li></ol></p>"
           "<p><strong>Note:</strong> Aliases can also be defined from the command line in the main profile window "
           "like this:</p>"
           "<p><code>lua permAlias(&quot;my greets&quot;, &quot;&quot;, &quot;^hi$&quot;, [[send (&quot;say Greetings, "
           "traveller!&quot;) echo (&quot;We said hi!&quot;)]])</code></p>"
    ...
    msgInfoAddTrigger =
        tr("<p>Triggers react on game output. To add a new trigger:"
           "<ol><li>Click on the 'Add Item' icon above.</li>"
           "<li>Define a <strong>pattern</strong> that you want to trigger on.</li>"
           "<li>Select the appropriate pattern <strong>type</strong>.</li>"
           "<li>Define a clear text <strong>command</strong> that you want to send to the game if the trigger finds "
           "the pattern in the text from the game, or write a script for more complicated needs..</li>"
           "<li><strong>Activate</strong> the trigger.</li></ol></p>"
           "<p><strong>Note:</strong> Triggers can also be defined from the command line in the main profile window "
           "like this:</p>"
           "<p><code>lua permSubstringTrigger(&quot;My drink trigger&quot;, &quot;&quot;, &quot;You are "
           "thirsty.&quot;, function() send(&quot;drink water&quot;) end)</code></p>"
           "<p>This will keep you refreshed.</p>"
           "<p>Check the manual for <a href='http://wiki.mudlet.org/w/Manual:Contents'>more information</a>.</p>");

    msgInfoAddScript =
        tr("<p>Scripts organize code and can react to events. To add a new script:"
           "<ol><li>Click on the 'Add Item' icon above.</li>"
           "<li>Enter a script in the box below. You can for example define <strong>functions</strong> to be called by "
           "other triggers, aliases, etc.</li>"
           "<li>If you write lua <strong>commands</strong> without defining a function, they will be run on Mudlet "
           "startup and each time you open the script for editing.</li>"
           "<li>If needed, you can register a list of <strong>events</strong> with the + and - symbols. If one of "
           "these events take place, the function with the same name as the script item itself will be called.</li>"
           ...
           "<p><strong>Note:</strong> Events can also be added to a script from the command line in the main profile "
           "window like this:</p>"
           "<p><code>lua registerAnonymousEventHandler(&quot;nameOfTheMudletEvent&quot;, "
           "&quot;nameOfYourFunctionToBeCalled&quot;)</code></p>"
           "<p>Check the manual for <a href='http://wiki.mudlet.org/w/Manual:Contents'>more information</a>.</p>");

    msgInfoAddTimer =
        tr("<p>Timers react after a timespan once or regularly. To add a new timer:"
           "<ol><li>Click on the 'Add Item' icon above.</li>"
           "<li>Define the <strong>timespan</strong> after which the timer should react in a this format: hours : "
           "minutes : seconds.</li>"
           "<li>Define a clear text <strong>command</strong> that you want to send to the game when the time has "
           "passed, or write a script for more complicated needs.</li>"
           "<li><strong>Activate</strong> the timer.</li></ol></p>"
           "<p><strong>Note:</strong> If you want the trigger to react only once and not regularly, use the Lua "
           "tempTimer() function instead.</p>"
           "<p><strong>Note:</strong> Timers can also be defined from the command line in the main profile window like "
           "this:</p>"
    ...
    msgInfoAddButton =
        tr("<p>Buttons react on mouse clicks. To add a new button:"
           ...
           "<li>Define a clear text <strong>command</strong> that you want to send to the game if the button is "
           "pressed, or write a script for more complicated needs.</li>"
           "<li><strong>Activate</strong> the toolbar, menu or button. </li></ol>"
           "<p><strong>Note:</strong> Deactivated items will be hidden and if they are toolbars or menus then all the "
           "items they contain will be also be hidden.</p>"
           "<p><strong>Note:</strong> If a button is made a <strong>click-down</strong> button then you may also "
           "define a clear text command that you want to send to the game when the button is pressed a second time to "
           "uncheck it or to write a script to run when it happens - within such a script the Lua 'getButtonState()' "
           "function reports whether the button is up or down.</p>"
           "<p>Check the manual for <a href='http://wiki.mudlet.org/w/Manual:Contents'>more information</a>.</p>");

    msgInfoAddKey =
        tr("<p>Keys react on keyboard presses. To add a new key binding:"
           "<ol><li>Click on the 'Add Item' icon above.</li>"
           "<li>Click on <strong>'grab key'</strong> and then press your key combination, e.g. including modifier keys "
           "like Control, Shift, etc.</li>"
           "<li>Define a clear text <strong>command</strong> that you want to send to the game if the button is "
           "pressed, or write a script for more complicated needs.</li>"
           "<li><strong>Activate</strong> the new key binding.</li></ol></p>"
           "<p><strong>Note:</strong> Keys can also be defined from the command line in the main profile window like "
           "this:</p>"
           "<p><code>lua permKey(&quot;my jump key&quot;, &quot;&quot;, mudlet.key.F8, [[send(&quot;jump&quot;]]) "
           "end)</code></p>"
           "<p>Pressing F8 will make you jump.</p>"
           "<p>Check the manual for <a href='http://wiki.mudlet.org/w/Manual:Contents'>more information</a>.</p>");

    msgInfoAddVar = tr(
        "<p>Variables store information. To make a new variable:"
        ...
        "<li>If you want to keep the variable in your next Mudlet sessions, check the checkbox in the list of "
        "variables to the left.</li>"
        "<li>To remove a variable manually, set it to 'nil' or click on the 'Delete' icon above.</li></ol></p>"
        "<p><strong>Note:</strong> Variables created here won't be saved when Mudlet shuts down unless you check their "
        "checkbox in the list of variables to the left. You could also create scripts with the variables instead.</p>"
        "<p><strong>Note:</strong> Variables and tables can also be defined from the command line in the main profile "
        "window like this:</p>"
    ...
    connect(mpSystemMessageArea->messageAreaCloseButton, &QAbstractButton::clicked, mpSystemMessageArea,
            &QWidget::hide);

    // main areas
    mpTriggersMainArea = new dlgTriggersMainArea(this);
    layoutColumn->addWidget(mpTriggersMainArea, 1);
    connect(mpTriggersMainArea->pushButtonFgColor, &QAbstractButton::clicked, this,
            &dlgTriggerEditor::slot_colorizeTriggerSetFgColor);
    connect(mpTriggersMainArea->pushButtonBgColor, &QAbstractButton::clicked, this,
            &dlgTriggerEditor::slot_colorizeTriggerSetBgColor);
    connect(mpTriggersMainArea->pushButtonSound, &QAbstractButton::clicked, this, &dlgTriggerEditor::slot_soundTrigger);
    connect(mpTriggersMainArea->groupBox_triggerColorizer, &QGroupBox::clicked, this,
            &dlgTriggerEditor::slot_toggleGroupBoxColorizeTrigger);
    connect(mpTriggersMainArea->toolButton_clearSoundFile, &QAbstractButton::clicked, this,
            &dlgTriggerEditor::slot_clearSoundFile);
    ...
    connect(mpActionsMainArea->checkBox_action_button_isPushDown, &QCheckBox::stateChanged, this,
            &dlgTriggerEditor::slot_toggle_isPushDownButton);
    ...
    connect(mpScriptsMainArea->lineEdit_script_event_handler_entry, &QLineEdit::returnPressed, this,
            &dlgTriggerEditor::slot_script_main_area_add_handler);
    connect(mpScriptsMainArea->listWidget_script_registered_event_handlers, &QListWidget::itemClicked, this,
            &dlgTriggerEditor::slot_script_main_area_edit_handler);
    ...
    connect(mpSourceEditorEdbee->controller(), &edbee::TextEditorController::updateStatusTextSignal, this,
            &dlgTriggerEditor::slot_updateStatusBar);
    ...
    connect(mudlet::self(), &mudlet::signal_editorTextOptionsChanged, this,
            &dlgTriggerEditor::slot_changeEditorTextOptions);
    ...
    connect(mpSourceEditorFindArea->lineEdit_findText, &QLineEdit::textChanged, this,
            &dlgTriggerEditor::slot_source_find_text_changed);
    connect(mpSourceEditorFindArea, &dlgSourceEditorFindArea::signal_sourceEditorMovementNecessary, this,
            &dlgTriggerEditor::slot_move_source_find);
    connect(mpSourceEditorFindArea->pushButton_findPrevious, &QPushButton::clicked, this,
            &dlgTriggerEditor::slot_source_find_previous);
    connect(mpSourceEditorFindArea->pushButton_findNext, &QPushButton::clicked, this,
            &dlgTriggerEditor::slot_source_find_next);
    connect(mpSourceEditorFindArea, &dlgSourceEditorFindArea::signal_sourceEditorFindPrevious, this,
            &dlgTriggerEditor::slot_source_find_previous);
    connect(mpSourceEditorFindArea, &dlgSourceEditorFindArea::signal_sourceEditorFindNext, this,
            &dlgTriggerEditor::slot_source_find_next);
    connect(mpSourceEditorFindArea->pushButton_close, &QPushButton::clicked, this,
            &dlgTriggerEditor::slot_close_source_find);
    ...
    connect(mpSourceEditorEdbee->textEditorComponent(), &QWidget::customContextMenuRequested, this,
            &dlgTriggerEditor::slot_editorContextMenu);
    ...
    button_toggleSearchAreaResults->setStyleSheet(
        QStringLiteral("QToolButton::on {border-image: url(:/icons/arrow-down_grey.png);} "
    ...
    connect(button_toggleSearchAreaResults, &QAbstractButton::clicked, this,
            &dlgTriggerEditor::slot_showSearchAreaResults);

    connect(mpTriggersMainArea->toolButton_toggleExtraControls, &QAbstractButton::clicked, this,
            &dlgTriggerEditor::slot_showAllTriggerControls);
    ...
    QAction *viewScriptsAction =
        new QAction(QIcon(QStringLiteral(":/icons/document-properties.png")), tr("Scripts"), this);
    ...
    QAction *viewKeysAction =
        new QAction(QIcon(QStringLiteral(":/icons/preferences-desktop-keyboard.png")), tr("Keys"), this);
    ...
    QAction *toggleActiveAction =
        new QAction(QIcon(QStringLiteral(":/icons/document-encrypt.png")), tr("Activate"), this);
    ...
    QAction *deleteTriggerAction = new QAction(
        QIcon::fromTheme(QStringLiteral(":/icons/edit-delete"), QIcon(QStringLiteral(":/icons/edit-delete.png"))),
        tr("Delete Item"), this);
    ...
    saveAction->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                               .arg(tr("Saves the selected item. (Ctrl+S)</p>Saving causes any changes to the item to "
                                       "take effect.\nIt will not save to disk, "
                                       "so changes will be lost in case of a computer/program crash (but Save Profile "
                                       "to the right will be secure.)")));
    saveAction->setStatusTip(tr("Saves the selected trigger, script, alias, etc, causing new changes to take effect - "
                                "does not save to disk though..."));
    ...
    mProfileSaveAction->setToolTip(
        QStringLiteral("<html><head/><body><p>%1</p></body></html>")
            .arg(tr(
                R"(Saves your profile. (Ctrl+Shift+S)<p>Saves your entire profile (triggers, aliases, scripts, timers, buttons and keys, but not the map or script-specific settings) to your computer disk, so in case of a computer or program crash, all changes you have done will be retained.</p><p>It also makes a backup of your profile, you can load an older version of it when connecting.</p><p>Should there be any modules that are marked to be "<i>synced</i>" this will also cause them to be saved and reloaded into other profiles if they too are active.)")));
    mProfileSaveAction->setStatusTip(tr(
        R"(Saves your entire profile (triggers, aliases, scripts, timers, buttons and keys, but not the map or script-specific settings); also "synchronizes" modules that are so marked.)"));
    connect(mProfileSaveAction, &QAction::triggered, this, &dlgTriggerEditor::slot_profileSaveAction);

    mProfileSaveAsAction =
        new QAction(QIcon(QStringLiteral(":/icons/utilities-file-archiver.png")), tr("Save Profile As"), this);
    ...
    QAction *viewStatsAction =
        new QAction(QIcon(QStringLiteral(":/icons/view-statistics.png")), tr("Statistics"), this);
    ...
    QAction *showDebugAreaAction =
        new QAction(QIcon(QStringLiteral(":/icons/tools-report-bug.png")), tr("Debug"), this);
    showDebugAreaAction->setToolTip(tr("Activates Debug Messages -> system will be <b><i>slower</i></b>."));
    showDebugAreaAction->setStatusTip(
        tr("Shows/Hides the separate Central Debug Console - when being displayed the system will be slower."));
    ...
    connect(checkBox_displayAllVariables, &QAbstractButton::toggled, this,
            &dlgTriggerEditor::slot_toggleHiddenVariables);

    connect(mpVarsMainArea->checkBox_variable_hidden, &QAbstractButton::clicked, this,
            &dlgTriggerEditor::slot_toggleHiddenVar);
    ...
    connect(comboBox_searchTerms, qOverload<const QString &>(&QComboBox::activated), this,
            &dlgTriggerEditor::slot_searchMudletItems);
    connect(treeWidget_triggers, &QTreeWidget::itemClicked, this, &dlgTriggerEditor::slot_trigger_selected);
    connect(treeWidget_triggers, &QTreeWidget::itemSelectionChanged, this,
            &dlgTriggerEditor::slot_tree_selection_changed);
    ...
    connect(treeWidget_variables, &QTreeWidget::itemSelectionChanged, this,
            &dlgTriggerEditor::slot_tree_selection_changed);
    connect(treeWidget_searchResults, &QTreeWidget::itemClicked, this,
            &dlgTriggerEditor::slot_item_selected_search_list);
    ...
    button_toggleSearchAreaResults->setMaximumSize(
        QSize((3 * comboBox_searchTerms->height()) / 4, (3 * comboBox_searchTerms->height()) / 4));
    button_toggleSearchAreaResults->setMinimumSize(
        QSize((3 * comboBox_searchTerms->height()) / 4, (3 * comboBox_searchTerms->height()) / 4));
    ...
    for (auto child : pLineEdit_searchTerm->children())
    {
        ...
        {
            connect(pAction_clear, &QAction::triggered, this, &dlgTriggerEditor::slot_clearSearchResults,
                    Qt::QueuedConnection);
            break;
        }
    }
    ...
    mpAction_searchCaseSensitive->setToolTip(QStringLiteral("<p>%1</p>").arg(tr("Match case precisely")));
    ...
    mpAction_searchIncludeVariables->setToolTip(QStringLiteral("<p>%1</p>").arg(tr("Search variables (slower)")));
    ...
    connect(mpAction_searchCaseSensitive, &QAction::triggered, this,
            &dlgTriggerEditor::slot_toggleSearchCaseSensitivity);
    connect(mpAction_searchIncludeVariables, &QAction::triggered, this,
            &dlgTriggerEditor::slot_toggleSearchIncludeVariables);
    ...
    connect(mpScriptsMainArea->toolButton_script_add_event_handler, &QAbstractButton::pressed, this,
            &dlgTriggerEditor::slot_script_main_area_add_handler);
    connect(mpScriptsMainArea->toolButton_script_remove_event_handler, &QAbstractButton::pressed, this,
            &dlgTriggerEditor::slot_script_main_area_delete_handler);
    ...
    patternList << tr("substring") << tr("perl regex") << tr("start of line") << tr("exact match") << tr("lua function")
                << tr("line spacer") << tr("color trigger") << tr("prompt");
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        connect(pBox, qOverload<int>(&QComboBox::currentIndexChanged), this,
                &dlgTriggerEditor::slot_setupPatternControls);
...
void dlgTriggerEditor::readSettings()
{
    ...
    QSettings settings((settings_new.contains("pos") ? "mudlet" : "Mudlet"),
...
void dlgTriggerEditor::slot_item_selected_search_list(QTreeWidgetItem *pItem)
{
    ...
    case EditorViewType::cmTriggerView: { // DONE
        ...
        foundItemsList = treeWidget_triggers->findItems(
            pItem->data(0, NameRole).toString(), Qt::MatchCaseSensitive | Qt::MatchFixedString | Qt::MatchRecursive, 0);
        ...
        for (auto treeWidgetItem : qAsConst(foundItemsList))
        {
            ...
            if (treeWidgetItem->data(0, IdRole).toInt() == idSearch)
            {
                ...
                case SearchResultIsScript:
                    ...
                    controller->moveCaretTo(pItem->data(0, PatternOrLineRole).toInt(),
                                            pItem->data(0, PositionRole).toInt(), false);
                    break;
                ...
                case SearchResultIsPattern: {
                    dlgTriggerPatternEdit *pTriggerPattern =
                        mTriggerPatternEdit.at(pItem->data(0, PatternOrLineRole).toInt());
                ...
                case SearchResultIsCommand:
                    ...
                    mpTriggersMainArea->lineEdit_trigger_command->setCursorPosition(
                        pItem->data(0, PositionRole).toInt());
                    break;
                default:
                    qDebug() << "dlgTriggerEditor::slot_item_selected_list(...) Called for a TRIGGER type item but "
                                "handler for element of type:"
    ...
    case EditorViewType::cmAliasView: {
        foundItemsList = treeWidget_aliases->findItems(
            pItem->data(0, NameRole).toString(), Qt::MatchCaseSensitive | Qt::MatchFixedString | Qt::MatchRecursive, 0);
        ...
        for (auto treeWidgetItem : qAsConst(foundItemsList))
        {
            ...
            if (treeWidgetItem->data(0, IdRole).toInt() == idSearch)
            {
                ...
                case SearchResultIsCommand:
                    ...
                    break;
                default:
                    qDebug() << "dlgTriggerEditor::slot_item_selected_list(...) Called for a ALIAS type item but "
                                "handler for element of type:"
                             << treeWidgetItem->data(0, TypeRole).toInt() << "not yet done/applicable...!";
                }
                return;
    ...
    case EditorViewType::cmScriptView: {
        foundItemsList = treeWidget_scripts->findItems(
            pItem->data(0, NameRole).toString(), Qt::MatchCaseSensitive | Qt::MatchFixedString | Qt::MatchRecursive, 0);
        ...
        for (auto treeWidgetItem : qAsConst(foundItemsList))
        {
            ...
            if (treeWidgetItem->data(0, IdRole).toInt() == idSearch)
            {
                ...
                case SearchResultIsEventHandler:
                    mpScriptsMainArea->listWidget_script_registered_event_handlers->setCurrentRow(
                        pItem->data(0, PatternOrLineRole).toInt(), QItemSelectionModel::Clear);
                    mpScriptsMainArea->listWidget_script_registered_event_handlers->scrollTo(
                        mpScriptsMainArea->listWidget_script_registered_event_handlers->currentIndex());
                    // Taken from slot_script_main_area_edit_handler():
                    // Note the handler item being edited:
                    mpScriptsMainAreaEditHandlerItem =
                        mpScriptsMainArea->listWidget_script_registered_event_handlers->currentItem();
                    // Copy the event name to the entry widget:
                    mpScriptsMainArea->lineEdit_script_event_handler_entry->setText(
                        mpScriptsMainAreaEditHandlerItem->text());
                    // Activate editing flag:
                    mIsScriptsMainAreaEditHandler = true;
                    break;
                default:
                    qDebug() << "dlgTriggerEditor::slot_item_selected_list(...) Called for a SCRIPT type item but "
                                "handler for element of type:"
                             << treeWidgetItem->data(0, TypeRole).toInt() << "not yet done/applicable...!";
                }
    ...
    case EditorViewType::cmActionView: {
        foundItemsList = treeWidget_actions->findItems(
            pItem->data(0, NameRole).toString(), Qt::MatchCaseSensitive | Qt::MatchFixedString | Qt::MatchRecursive, 0);
        ...
        for (auto treeWidgetitem : qAsConst(foundItemsList))
        {
            ...
            if (treeWidgetitem->data(0, IdRole).toInt() == idSearch)
            {
                ...
                case SearchResultIsCommand:
                    ...
                    mpActionsMainArea->lineEdit_action_button_command_down->setCursorPosition(
                        pItem->data(0, PositionRole).toInt());
                    break;
                case SearchResultIsExtraCommand:
                    if (mpActionsMainArea->checkBox_action_button_isPushDown->isChecked())
                    {
                        ...
                        mpActionsMainArea->lineEdit_action_button_command_up->setCursorPosition(
                            pItem->data(0, PositionRole).toInt());
                    }
                    break;
                case SearchResultsIsCss: {
                    ...
                    if (pItem->data(0, PatternOrLineRole).toInt())
                    {
                        ...
                        cssCursor.movePosition(QTextCursor::Down, QTextCursor::MoveAnchor,
                                               pItem->data(0, PatternOrLineRole).toInt());
                    }
                    if (pItem->data(0, PositionRole).toInt())
                    {
                        // Are we not on the first character - if so move right that many QChars...
                        cssCursor.movePosition(QTextCursor::Right, QTextCursor::MoveAnchor,
                                               pItem->data(0, PositionRole).toInt());
                    }
                ...
                break;
                default:
                    qDebug() << "dlgTriggerEditor::slot_item_selected_list(...) Called for a BUTTON type item but "
                                "handler for element of type:"
                             << treeWidgetitem->data(0, TypeRole).toInt() << "not yet done/applicable...!";
                } // End or switch()
                return;
    ...
    case EditorViewType::cmTimerView: {
        foundItemsList = treeWidget_timers->findItems(
            pItem->data(0, NameRole).toString(), Qt::MatchCaseSensitive | Qt::MatchFixedString | Qt::MatchRecursive, 0);
        ...
        for (auto treeWidgetItem : qAsConst(foundItemsList))
        {
            ...
            if (treeWidgetItem->data(0, IdRole).toInt() == idSearch)
            {
                ...
                case SearchResultIsCommand:
                    ...
                    break;
                default:
                    qDebug() << "dlgTriggerEditor::slot_item_selected_list(...) Called for a TIMER type item but "
                                "handler for element of type:"
                             << treeWidgetItem->data(0, TypeRole).toInt() << "not yet done/applicable...!";
                } // End of switch()
                return;
    ...
    case EditorViewType::cmKeysView: {
        foundItemsList = treeWidget_keys->findItems(
            pItem->data(0, NameRole).toString(), Qt::MatchCaseSensitive | Qt::MatchFixedString | Qt::MatchRecursive, 0);
        ...
        for (auto treeWidgetItem : qAsConst(foundItemsList))
        {
            ...
            if (idTree == idSearch)
            {
                ...
                case SearchResultIsCommand:
                    ...
                    break;
                default:
                    qDebug() << "dlgTriggerEditor::slot_item_selected_list(...) Called for a KEY type item but handler "
                                "for element of type:"
                             << treeWidgetItem->data(0, TypeRole).toInt() << "not yet done/applicable...!";
                } // End of switch()
                return;
        ...
        while (it.hasNext())
        {
            ...
            if (vu->shortVarName(var) == varShort)
            {
                ...
                case SearchResultIsValue:
                    ...
                    break;
                default:
                    qDebug() << "dlgTriggerEditor::slot_item_selected_list(...) Called for a VAR type item but handler "
                                "for element of type:"
                             << treeWidgetItem->data(0, TypeRole).toInt() << "not yet done/applicable...!";
                }
                return;
...
void dlgTriggerEditor::searchVariables(const QString &s)
{
    ...
    while (itBaseVarChildren.hasNext())
    {
        ...
        while (itVarDecendent.hasNext())
        {
            ...
            int startPos = 0;
            if ((startPos = name.indexOf(
                     s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) !=
                -1)
            {
                QStringList sl;
            ...
            // tree widget...
            if (value != QLatin1String("function") &&
...
void dlgTriggerEditor::searchKeys(const QString &s)
{
    ...
    for (auto key : nodes)
    {
        ...
        if ((startPos = name.indexOf(
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            QStringList sl;
        ...
        if ((startPos = key->getCommand().indexOf(
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            QStringList sl;
            if (!parent)
            {
                ...
                parent = new QTreeWidgetItem(sl);
                setAllSearchData(parent, EditorViewType::cmKeysView, name, key->getID(), SearchResultIsCommand,
                                 startPos);
                treeWidget_searchResults->addTopLevelItem(parent);
            }
            else
            {
                ...
                setAllSearchData(pItem, EditorViewType::cmKeysView, name, key->getID(), SearchResultIsCommand,
                                 startPos);
                parent->addChild(pItem);
                parent->setExpanded(true);
            }
        ...
        for (int index = 0; index < total; ++index)
        {
            ...
            if (textList.at(index).isEmpty() ||
                !textList.at(index).contains(
                    s, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive)))
            {
            ...
            int instance = 0;
            startPos = 0;
            while ((startPos = textList.at(index).indexOf(
                        s, startPos,
                        ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) !=
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmKeysView, name, key->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    ...
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmKeysView, name, key->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
    ...
    for (auto timer : nodes)
    {
                ...
                s, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive)))
        {
            ...
            if (!parent)
            {
                ...
                parent = new QTreeWidgetItem(sl);
                setAllSearchData(parent, EditorViewType::cmTimerView, name, timer->getID(), SearchResultIsCommand,
                                 startPos);
                treeWidget_searchResults->addTopLevelItem(parent);
            }
            else
            {
                sl << QString() << QString() << tr("Command");
                pItem = new QTreeWidgetItem(parent, sl);
                setAllSearchData(pItem, EditorViewType::cmTimerView, name, timer->getID(), SearchResultIsCommand,
                                 startPos);
                parent->addChild(pItem);
                parent->setExpanded(true);
            }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmTimerView, name, timer->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    sl << QString() << QString() << tr("Lua code (%1:%2)").arg(index + 1).arg(startPos + 1) << whatText;
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmTimerView, name, timer->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
...
void dlgTriggerEditor::searchActions(const QString &s)
{
    ...
    for (auto action : nodes)
    {
        ...
        // TODO: (A) Revise to count multiple instances of search string within command?
        if ((startPos = action->getCommandButtonDown().indexOf(
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            QStringList sl;
            if (!parent)
            {
                ...
                parent = new QTreeWidgetItem(sl);
                setAllSearchData(parent, EditorViewType::cmActionView, name, action->getID(), SearchResultIsCommand,
                                 startPos);
                treeWidget_searchResults->addTopLevelItem(parent);
            }
            else
            {
                ...
                pItem = new QTreeWidgetItem(parent, sl);
                setAllSearchData(pItem, EditorViewType::cmActionView, name, action->getID(), SearchResultIsCommand,
                                 startPos);
                parent->addChild(pItem);
                parent->setExpanded(true);
            }
        ...
        if (action->isPushDownButton())
        {
            ...
            if ((startPos = action->getCommandButtonUp().indexOf(
                     s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) !=
                -1)
            {
                QStringList sl;
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmActionView, name, action->getID(),
                                     SearchResultIsExtraCommand, startPos);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    ...
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmActionView, name, action->getID(),
                                     SearchResultIsExtraCommand, startPos);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    sl << tr("Action") << name << tr("Stylesheet {L: %1 C: %2}").arg(index + 1).arg(startPos + 1)
                       << textList.at(index);
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmActionView, name, action->getID(), SearchResultsIsCss,
                                     startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    sl << QString() << QString() << tr("Stylesheet {L: %1 C: %2}").arg(index + 1).arg(startPos + 1)
                       << textList.at(index);
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmActionView, name, action->getID(), SearchResultsIsCss,
                                     startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmActionView, name, action->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    sl << QString() << QString() << tr("Lua code (%1:%2)").arg(index + 1).arg(startPos + 1) << whatText;
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmActionView, name, action->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmScriptView, name, script->getID(),
                                     SearchResultIsEventHandler, startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    ...
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmScriptView, name, script->getID(),
                                     SearchResultIsEventHandler, startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmScriptView, name, script->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    sl << QString() << QString() << tr("Lua code (%1:%2)").arg(index + 1).arg(startPos + 1) << whatText;
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmScriptView, name, script->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
...
void dlgTriggerEditor::searchAliases(const QString &s)
{
    ...
    for (auto alias : nodes)
    {
        ...
        // The simple "command"
        if ((startPos = alias->getCommand().indexOf(
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            QStringList sl;
            if (!parent)
            {
                ...
                parent = new QTreeWidgetItem(sl);
                setAllSearchData(parent, EditorViewType::cmAliasView, name, alias->getID(), SearchResultIsCommand,
                                 startPos);
                treeWidget_searchResults->addTopLevelItem(parent);
            }
            else
            {
                sl << QString() << QString() << tr("Command");
                pItem = new QTreeWidgetItem(parent, sl);
                setAllSearchData(pItem, EditorViewType::cmAliasView, name, alias->getID(), SearchResultIsCommand,
                                 startPos);
                parent->addChild(pItem);
                parent->setExpanded(true);
            }
        ...
        if ((startPos = alias->getRegexCode().indexOf(
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            QStringList sl;
            if (!parent)
            {
                ...
                parent = new QTreeWidgetItem(sl);
                setAllSearchData(parent, EditorViewType::cmAliasView, name, alias->getID(), SearchResultIsPattern,
                                 startPos);
                treeWidget_searchResults->addTopLevelItem(parent);
            }
            else
            {
                ...
                pItem = new QTreeWidgetItem(parent, sl);
                setAllSearchData(pItem, EditorViewType::cmAliasView, name, alias->getID(), SearchResultIsPattern,
                                 startPos);
                parent->addChild(pItem);
                parent->setExpanded(true);
            }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmAliasView, name, alias->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    sl << QString() << QString() << tr("Lua code (%1:%2)").arg(index + 1).arg(startPos + 1) << whatText;
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmAliasView, name, alias->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
...
void dlgTriggerEditor::searchTriggers(const QString &s)
{
    ...
    for (auto trigger : nodes)
    {
                 ...
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            ...
            // This part can never have a parent as it is the first part of this item
            parent = new QTreeWidgetItem(sl);
            setAllSearchData(parent, EditorViewType::cmTriggerView, name, trigger->getID(), SearchResultIsName,
                             startPos);
            treeWidget_searchResults->addTopLevelItem(parent);
        }
        ...
        // The simple "command"
        // TODO: (A) Revise to count multiple instances of search string within command?
        if ((startPos = trigger->getCommand().indexOf(
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            QStringList sl;
            if (!parent)
            {
                ...
                parent = new QTreeWidgetItem(sl);
                setAllSearchData(parent, EditorViewType::cmTriggerView, name, trigger->getID(), SearchResultIsCommand,
                                 startPos);
                treeWidget_searchResults->addTopLevelItem(parent);
            }
            else
            {
                sl << QString() << QString() << tr("Command");
                pItem = new QTreeWidgetItem(parent, sl);
                setAllSearchData(pItem, EditorViewType::cmTriggerView, name, trigger->getID(), SearchResultIsCommand,
                                 startPos);
                parent->addChild(pItem);
                parent->setExpanded(true);
            }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmTriggerView, name, trigger->getID(),
                                     SearchResultIsPattern, startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    ...
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmTriggerView, name, trigger->getID(),
                                     SearchResultIsPattern, startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
        ...
        for (int index = 0; index < total; ++index)
        {
                   ...
                   -1)
            {
                ...
                if (!parent)
                {
                    ...
                    parent = new QTreeWidgetItem(sl);
                    setAllSearchData(parent, EditorViewType::cmTriggerView, name, trigger->getID(),
                                     SearchResultIsScript, startPos, index, instance++);
                    treeWidget_searchResults->addTopLevelItem(parent);
                }
                else
                {
                    sl << QString() << QString() << tr("Lua code (%1:%2)").arg(index + 1).arg(startPos + 1) << whatText;
                    pItem = new QTreeWidgetItem(parent, sl);
                    setAllSearchData(pItem, EditorViewType::cmTriggerView, name, trigger->getID(), SearchResultIsScript,
                                     startPos, index, instance++);
                    parent->addChild(pItem);
                    parent->setExpanded(true);
                }
...
void dlgTriggerEditor::recursiveSearchTimers(TTimer *pTriggerParent, const QString &s)
{
    ...
    for (auto timer : *childrenList)
    {
        ...
        // The simple "command"
        // TODO: (A) Revise to count multiple instances of search string within command?
        if ((startPos = timer->getCommand().indexOf(
                 s, 0, ((mSearchOptions & SearchOptionCaseSensitive) ? Qt::CaseSensitive : Qt::CaseInsensitive))) != -1)
        {
            QStringList sl;
            if (!parent)
...
void dlgTriggerEditor::activeToggle_trigger()
{
    ...
    else
    {
        if (pT->isActive())
        {
            ...
            {
                icon.addPixmap(QPixmap(QStringLiteral(":/icons/tag_checkbox_checked_grey.png")), QIcon::Normal,
                               QIcon::Off);
            }
        }
        else
    ...
    if (pT->state())
    {
        ...
        {
            showInfo(
                tr(R"(Trying to activate a trigger group, filter or trigger or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
        else
        {
            showInfo(
                tr(R"(Trying to deactivate a trigger group, filter or trigger or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
    }
    else
    {
        ...
        showError(
            tr(R"(<b>Unable to activate a filter or trigger or the part of a module "<tt>%1</tt>" that contains them; reason: %2.</b></p>
        ...
        if (pT->state())
        {
            if (pT->isFilterChain())
            {
                ...
                else
                {
                    ...
                    {
                        icon.addPixmap(QPixmap(QStringLiteral(":/icons/filter-grey-locked.png")), QIcon::Normal,
                                       QIcon::Off);
                    }
                }
            }
            else if (pT->isFolder())
            {
                ...
                else
                {
                    if (pT->ancestorsActive())
                    {
                        icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-blue-locked.png")), QIcon::Normal,
                                       QIcon::Off);
                    }
                    else
                    {
                        icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-grey-locked.png")), QIcon::Normal,
                                       QIcon::Off);
                    }
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        icon.addPixmap(QPixmap(QStringLiteral(":/icons/tag_checkbox-grey.png")), QIcon::Normal,
                                       QIcon::Off);
                    }
                }
            }
...
void dlgTriggerEditor::activeToggle_timer()
{
    ...
    if (pT->state())
    {
        ...
        {
            showInfo(
                tr(R"(Trying to activate a timer group, offset timer, timer or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
        else
        {
            showInfo(
                tr(R"(Trying to deactivate a timer group, offset timer, timer or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
    }
    else
    {
        pT->setIsActive(false);
        showError(
            tr(R"(<p><b>Unable to activate an offset timer or timer or the part of a module "<tt>%1</tt>" that contains them; reason: %2.</b></p>
                     <p><i>You will need to reactivate this after the problem has been corrected.</i></p>)")
                .arg(pT->getName(), pT->getError()));
        icon.addPixmap(QPixmap(QStringLiteral(":/icons/tools-report-bug.png")), QIcon::Normal, QIcon::Off);
    }
...
void dlgTriggerEditor::activeToggle_alias()
{
    ...
    if (pT->state())
    {
        ...
        {
            showInfo(
                tr(R"(Trying to activate an alias group, alias or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
        else
        {
            showInfo(
                tr(R"(Trying to deactivate an alias group, alias or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
    }
    else
    {
        pT->setIsActive(false);
        showError(
            tr(R"(<p><b>Unable to activate an alias or the part of a module "<tt>%1</tt>" that contains them; reason: %2.</b></p>
                     <p><i>You will need to reactivate this after the problem has been corrected.</i></p>)")
                .arg(pT->getName(), pT->getError()));
        icon.addPixmap(QPixmap(QStringLiteral(":/icons/tools-report-bug.png")), QIcon::Normal, QIcon::Off);
    }
        ...
        if (pT->state())
        {
            if (pT->isFolder())
            {
                ...
                else
                {
                    if (pT->ancestorsActive())
                    {
                        icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-violet-locked.png")), QIcon::Normal,
                                       QIcon::Off);
                    }
                    else
                    {
...
void dlgTriggerEditor::activeToggle_script()
{
    ...
    if (pT->state())
    {
        ...
        {
            showInfo(
                tr(R"(Trying to activate a script group, script or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
        else
        {
            showInfo(
                tr(R"(Trying to deactivate a script group, script or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
    }
    else
    {
        pT->setIsActive(false);
        showError(
            tr(R"(<p><b>Unable to activate a script group or script or the part of a module "<tt>%1</tt>" that contains them; reason: %2.</b></p>
                     <p><i>You will need to reactivate this after the problem has been corrected.</i></p>)")
                .arg(pT->getName(), pT->getError()));
        icon.addPixmap(QPixmap(QStringLiteral(":/icons/tools-report-bug.png")), QIcon::Normal, QIcon::Off);
    }
...
void dlgTriggerEditor::activeToggle_action()
{
    ...
    if (pT->state())
    {
        ...
        {
            showInfo(
                tr(R"(Trying to activate a button/menu/toolbar or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
        else
        {
            showInfo(
                tr(R"(Trying to deactivate a button/menu/toolbar or the part of a module "<tt>%1</tt>" that contains them <em>succeeded</em>.)")
                    .arg(pT->getName()));
        }
    }
    else
    {
        pT->setIsActive(false);
        showError(
            tr(R"(<p><b>Unable to activate a button/menu/toolbar or the part of a module "<tt>%1</tt>" that contains them; reason: %2.</b></p>
                     <p><i>You will need to reactivate this after the problem has been corrected.</i></p>)")
                .arg(pT->getName(), pT->getError()));
        icon.addPixmap(QPixmap(QStringLiteral(":/icons/tools-report-bug.png")), QIcon::Normal, QIcon::Off);
    }
...
void dlgTriggerEditor::activeToggle_key()
{
    ...
    showInfo(QString("Trying to %2 key <em>%1</em> %3.")
                 .arg(pT->getName(), pT->shouldBeActive() ? "activate" : "deactivate",
                      pT->state() ? "succeeded" : QString("failed; reason: ") + pT->getError()));
    if (pItem->childCount() > 0)
    {
        ...
        if (pT->state())
        {
            if (pT->isFolder())
            {
                ...
                else
                {
                    if (pT->ancestorsActive())
                    {
                        icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-pink-locked.png")), QIcon::Normal,
                                       QIcon::Off);
                    }
                    else
                    {
    ...
    if (pT)
    {
        ...
        else if (pT->isFolder())
        {
            if (!pT->mPackageName.isEmpty())
            {
                ...
                {
                    icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-brown-locked.png")), QIcon::Normal,
                                   QIcon::Off);
                }
            }
            else if (pT->isActive())
        ...
        if (pT->state())
        {
            ...
            if (old_name == tr("New trigger") || old_name == tr("New trigger group"))
            {
                ...
                {
                    _icon.addPixmap(QPixmap(QStringLiteral(":/icons/tag_checkbox_checked.png")), QIcon::Normal,
                                    QIcon::Off);
                }
    ...
    if (pT)
    {
        ...
        if (pT->isFolder())
        {
            ...
            else
            {
                ...
                {
                    icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-green-locked.png")), QIcon::Normal,
                                   QIcon::Off);
                }
            }
        }
...
void dlgTriggerEditor::saveAlias()
{
    ...
    if (!regex.isEmpty() && match.capturedStart() != -1)
    {
        ...
        pItem->setIcon(0, iconError);
        pItem->setText(0, name);
        showError(tr("Alias <em>%1</em> has an infinite loop - substitution matches its own pattern. Please fix it - "
                     "this alias isn't good as it'll call itself forever.")
                      .arg(name));
        return;
    }
        ...
        if (pT->state())
        {
            ...
            if (old_name == tr("New alias"))
            {
                ...
                if (pT->isFolder())
                {
                    if (pT->ancestorsActive())
                    {
                        _icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-violet.png")), QIcon::Normal,
                                        QIcon::Off);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        _icon.addPixmap(QPixmap(QStringLiteral(":/icons/tag_checkbox_checked_grey.png")), QIcon::Normal,
                                        QIcon::Off);
                    }
                }
                pItem->setIcon(0, _icon);
...
void dlgTriggerEditor::saveAction()
{
    ...
    if (pA)
    {
        ...
        if (pA->mLocation != location || pA->mOrientation != orientation ||
        ...
        if (pA->isFolder())
        {
            ...
            else if (!pA->getParent() || !pA->getParent()->mPackageName.isEmpty())
            {
                ...
                {
                    icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-yellow-locked.png")), QIcon::Normal,
                                   QIcon::Off);
                }
            }
            else
            {
                ...
                {
                    icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-cyan-locked.png")), QIcon::Normal,
                                   QIcon::Off);
                }
            }
        }
...
void dlgTriggerEditor::saveVar()
{
    ...
    mChangingVar = true;
    int uiNameType = mpVarsMainArea->comboBox_variable_key_type
                         ->itemData(mpVarsMainArea->comboBox_variable_key_type->currentIndex(), Qt::UserRole)
                         .toInt();
    int uiValueType = mpVarsMainArea->comboBox_variable_value_type
                          ->itemData(mpVarsMainArea->comboBox_variable_value_type->currentIndex(), Qt::UserRole)
                          .toInt();
    ...
    if (varRecast == 2)
    {
        ...
        else if (variable)
        {
            if (newName == variable->getName() &&
                (variable->getValueType() == LUA_TTABLE && newValue == variable->getValue()))
            {
                // no change made
            }
            else
            {
                ...
                if (variable->getValueType() != LUA_TTABLE &&
                    (newValue != variable->getValue() || uiValueType != variable->getValueType()))
                {
                    // lets check again
                    if (variable->getValueType() == LUA_TTABLE)
                    {
                    ...
                    else if (uiValueType == LUA_TBOOLEAN &&
            ...
            if (newValue != var->getValue() || uiValueType != var->getValueType())
            {
                ...
                else if (uiValueType == LUA_TNUMBER && newValue.toInt())
                {
                    uiValueType = LUA_TNUMBER;
                         ...
                         (newValue.toLower() == QLatin1String("true") || newValue.toLower() == QLatin1String("false")))
                {
                    uiValueType = LUA_TBOOLEAN;
                }
                else
...
void dlgTriggerEditor::setupPatternControls(const int type, dlgTriggerPatternEdit *pItem)
{
    ...
    case REGEX_PROMPT:
        ...
        else
        {
            ...
            pItem->pushButton_prompt->setToolTip(
                tr("A Go-Ahead (GA) signal from the game is required to make this feature work"));
        }
...
void dlgTriggerEditor::slot_setupPatternControls(int type)
{
    ...
    if (type == REGEX_COLOR_PATTERN)
    {
        if (pPatternItem->lineEdit_pattern->text().isEmpty())
        {
            ...
            pPatternItem->lineEdit_pattern->setText(
                TTrigger::createColorPatternText(TTrigger::scmIgnored, TTrigger::scmIgnored));
        }
             ...
             pPatternItem->lineEdit_pattern->text().endsWith(QLatin1String("}"))))
        {
            ...
            if (textAnsiFg == TTrigger::scmIgnored)
            {
                ...
                pPatternItem->pushButton_fgColor->setText(
                    tr("Foreground color ignored",
            ...
            else if (textAnsiFg == TTrigger::scmDefault)
            {
                pPatternItem->pushButton_fgColor->setStyleSheet(QString());
                pPatternItem->pushButton_fgColor->setText(
                    tr("Default foreground color",
                       "Color trigger default foreground color button, ensure all three instances have the same text"));
            }
            else
            {
                pPatternItem->pushButton_fgColor->setStyleSheet(
                    generateButtonStyleSheet(mpHost->getAnsiColor(textAnsiFg, false)));
                pPatternItem->pushButton_fgColor->setText(
                    tr("Foreground color [ANSI %1]",
            ...
            if (textAnsiBg == TTrigger::scmIgnored)
            {
                ...
                pPatternItem->pushButton_bgColor->setText(
                    tr("Background color ignored",
            ...
            {
                pPatternItem->pushButton_bgColor->setStyleSheet(
                    generateButtonStyleSheet(mpHost->getAnsiColor(textAnsiBg, true)));
                pPatternItem->pushButton_bgColor->setText(
                    tr("Background color [ANSI %1]",
...
void dlgTriggerEditor::slot_trigger_selected(QTreeWidgetItem *pItem)
{
    ...
    if (pT)
    {
            ...
            else if (pType == REGEX_COLOR_PATTERN)
            {
                ...
                if (pT->mColorPatternList.at(i))
                {
                    if (pT->mColorPatternList.at(i)->ansiFg == TTrigger::scmIgnored)
                    {
                        pPatternItem->pushButton_fgColor->setStyleSheet(QString());
                        pPatternItem->pushButton_fgColor->setText(
                            tr("Foreground color ignored", "Color trigger ignored foreground color button, ensure all "
                                                           "three instances have the same text"));
                    }
                    else if (pT->mColorPatternList.at(i)->ansiFg == TTrigger::scmDefault)
                    {
                        pPatternItem->pushButton_fgColor->setStyleSheet(QString());
                        pPatternItem->pushButton_fgColor->setText(
                            tr("Default foreground color", "Color trigger default foreground color button, ensure all "
                                                           "three instances have the same text"));
                    }
                    else
                    {
                        pPatternItem->pushButton_fgColor->setStyleSheet(
                            generateButtonStyleSheet(pT->mColorPatternList.at(i)->mFgColor));
                        pPatternItem->pushButton_fgColor->setText(
                            tr("Foreground color [ANSI %1]", "Color trigger ANSI foreground color button, ensure all "
                                                             "three instances have the same text")
                    ...
                    if (pT->mColorPatternList.at(i)->ansiBg == TTrigger::scmIgnored)
                    {
                        pPatternItem->pushButton_bgColor->setStyleSheet(QString());
                        pPatternItem->pushButton_bgColor->setText(
                            tr("Background color ignored", "Color trigger ignored background color button, ensure all "
                                                           "three instances have the same text"));
                    }
                    else if (pT->mColorPatternList.at(i)->ansiBg == TTrigger::scmDefault)
                    {
                        pPatternItem->pushButton_bgColor->setStyleSheet(QString());
                        pPatternItem->pushButton_bgColor->setText(
                            tr("Default background color", "Color trigger default background color button, ensure all "
                                                           "three instances have the same text"));
                    }
                    else
                    {
                        pPatternItem->pushButton_bgColor->setStyleSheet(
                            generateButtonStyleSheet(pT->mColorPatternList.at(i)->mBgColor));
                        pPatternItem->pushButton_bgColor->setText(
                            tr("Background color [ANSI %1]", "Color trigger ANSI background color button, ensure all "
                                                             "three instances have the same text")
                ...
                {
                    qWarning() << "dlgTriggerEditor::slot_trigger_selected(...) ERROR: TTrigger instance has an "
                                  "mColorPattern of size:"
                               << pT->mColorPatternList.size() << "but array element:" << i << "is a nullptr";
                    pPatternItem->pushButton_fgColor->setStyleSheet(QString());
        ...
        mpTriggersMainArea->lineEdit_soundFile->setCursorPosition(
            mpTriggersMainArea->lineEdit_soundFile->text().length());
        mpTriggersMainArea->toolButton_clearSoundFile->setEnabled(
        ...
        mpTriggersMainArea->pushButtonFgColor->setStyleSheet(
            generateButtonStyleSheet(pT->getFgColor(), pT->isColorizerTrigger()));
        mpTriggersMainArea->pushButtonFgColor->setProperty(cButtonBaseColor, pT->getFgColor().name());
        mpTriggersMainArea->pushButtonBgColor->setStyleSheet(
            generateButtonStyleSheet(pT->getBgColor(), pT->isColorizerTrigger()));
...
void dlgTriggerEditor::slot_timer_selected(QTreeWidgetItem *pItem)
{
    ...
    if (pT)
    {
        if (pT->getParent())
        {
            qDebug() << "[STATUS]: timer ID=" << pT->getID() << " name=" << pT->getName()
                     << " mActive = " << pT->isActive() << " mUserActiveState=" << pT->shouldBeActive()
                     << " parent=" << pT->getParent()->getName();
        }
        else
        {
            qDebug() << "[STATUS]: timer ID=" << pT->getID() << " name=" << pT->getName()
                     << "> mActive = " << pT->isActive() << " mUserActiveState=" << pT->shouldBeActive() << " parent=0";
        }
        QString command = pT->getCommand();
        QString name = pT->getName();
...
void dlgTriggerEditor::fillout_form()
{
    ...
    mpAliasBaseItem =
        new QTreeWidgetItem(static_cast<QTreeWidgetItem *>(nullptr), QStringList(tr("Aliases - Input Triggers")));
        ...
        if (script->state())
        {
            ...
            if (script->isFolder())
            {
                ...
                else
                {
                    ...
                    {
                        icon.addPixmap(QPixmap(QStringLiteral(":/icons/folder-orange-locked.png")), QIcon::Normal,
                                       QIcon::Off);
                    }
                }
            }
...
{
    mpScriptsMainArea->listWidget_script_registered_event_handlers->takeItem(
        mpScriptsMainArea->listWidget_script_registered_event_handlers->currentRow());
}
...
void dlgTriggerEditor::slot_next_section()
{
    ...
    {
    case EditorViewType::cmTriggerView:
        if (QStringLiteral("edbee::TextEditorComponent")
                .compare(QApplication::focusWidget()->metaObject()->className()) == 0)
        {
...
void dlgTriggerEditor::exportTrigger(const QString &fileName)
{
    ...
    if (pItem)
    {
        ...
        else
        {
            QMessageBox::warning(this, tr("Export Package:"),
                                 tr("You have to choose an item for export first. Please select a tree item and then "
    ...
    {
        QMessageBox::warning(this, tr("Export Package:"),
                             tr("You have to choose an item for export first. Please select a tree item and then click "
                                "on export again."));
        return;
    }
    XMLexport writer(pT);
...
{
    QString fileName =
        QFileDialog::getSaveFileName(this, tr("Export Triggers"), QDir::currentPath(), tr("Mudlet packages (*.xml)"));
    if (fileName.isEmpty())
    {
        return;
    }
...
void dlgTriggerEditor::slot_import()
{
    ...
    {
        QMessageBox::information(this, tr("Import Mudlet Package:"),
                                 tr("Package %1 is already installed.").arg(packageName));
        file.close();
        return;
    }
        ...
        fileName.endsWith(QStringLiteral(".mpackage"), Qt::CaseInsensitive))
    {
        ...
        // - the xml file must be located in the root directory of the zip package. example: myPack.zip contains: the
        // folder images and the file myPack.xml
...
{
    mSavingAs = true;
    QString fileName = QFileDialog::getSaveFileName(this, tr("Backup Profile"), QDir::homePath(),
...
{
    QString fileName =
        QFileDialog::getOpenFileName(this, tr("Seclect Icon"), QDir::homePath(), tr("Images (*.png *.xpm *.jpg)"));
...
void dlgTriggerEditor::slot_colorizeTriggerSetFgColor()
{
    ...
    auto color =
        QColorDialog::getColor(QColor(mpTriggersMainArea->pushButtonFgColor->property(cButtonBaseColor).toString()),
                               this, tr("Select foreground color to apply to matches"));
    if (color.isValid())
    {
...
void dlgTriggerEditor::slot_colorizeTriggerSetBgColor()
{
    ...
    auto color =
        QColorDialog::getColor(QColor(mpTriggersMainArea->pushButtonBgColor->property(cButtonBaseColor).toString()),
                               this, tr("Select background color to apply to matches"));
    if (color.isValid())
    {
...
void dlgTriggerEditor::slot_soundTrigger()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(
        ...
        tr("Audio files(*.aac *.mp3 *.mp4a *.oga *.ogg *.pcm *.wav *.wma);;"
           ...
           "All files(*.*)",
           // Intentional comment
           "This the list of file extensions that are considered for sounds from triggers, the terms inside of the "
           "'('...')' and the \";;\" are used programmatically and should not be changed."));
    if (!fileName.isEmpty())
    {
...
void dlgTriggerEditor::slot_color_trigger_fg()
{
    ...
    TTrigger::decodeColorPatternText(pPatternItem->lineEdit_pattern->text(), pT->mColorTriggerFgAnsi,
                                     pT->mColorTriggerBgAnsi);
    ...
    auto pD = new dlgColorTrigger(
        this, pT, false,
        tr("Select foreground trigger color for item %1").arg(QString::number(pPatternItem->mRow + 1)));
    pD->setModal(true);
    ...
    pPatternItem->lineEdit_pattern->setText(
        TTrigger::createColorPatternText(pT->mColorTriggerFgAnsi, pT->mColorTriggerBgAnsi));
...
void dlgTriggerEditor::slot_color_trigger_bg()
{
    ...
    // the other, non used (background in this method) part:
    auto pD = new dlgColorTrigger(
        this, pT, true, tr("Select background trigger color for item %1").arg(QString::number(pPatternItem->mRow + 1)));
    pD->setModal(true);
    // This sounds a bit iffy - prevent access to other application windows
    // whilst we get a colour setting:
    pD->setWindowModality(Qt::ApplicationModal);
...
void dlgTriggerEditor::slot_updateStatusBar(const QString &statusText)
{
    ...
    QRegularExpressionMatch match =
        simplifyEdbeeStatusBarRegex->match(statusText, 0, QRegularExpression::PartialPreferFirstMatch);
    QString stripped;
...
void dlgTriggerEditor::slot_changeEditorTextOptions(QTextOption::Flags state)
{
    ...
    config->beginChanges();
    config->setShowWhitespaceMode((state & QTextOption::ShowTabsAndSpaces) ? edbee::TextEditorConfig::ShowWhitespaces
                                                                           : edbee::TextEditorConfig::HideWhitespaces);
...
void dlgTriggerEditor::slot_toggleSearchCaseSensitivity(const bool state)
{
    ...
    {
        mSearchOptions =
            (mSearchOptions & ~(SearchOptionCaseSensitive)) | (state ? SearchOptionCaseSensitive : SearchOptionNone);
...
void dlgTriggerEditor::slot_toggleSearchIncludeVariables(const bool state)
{
    ...
    {
        mSearchOptions = (mSearchOptions & ~(SearchOptionIncludeVariables)) |
                         (state ? SearchOptionIncludeVariables : SearchOptionNone);
        createSearchOptionIcon();
        mpHost->mSearchOptions = mSearchOptions;
    }
}
...
void dlgTriggerEditor::slot_editorContextMenu()
{
    ...
    {
        menu->addAction(controller->createAction(
            "cut", tr("Cut"),
            QIcon::fromTheme(QStringLiteral("edit-cut"), QIcon(QStringLiteral(":/icons/edit-cut.png"))), menu));
        menu->addAction(controller->createAction(
            "copy", tr("Copy"),
            QIcon::fromTheme(QStringLiteral("edit-copy"), QIcon(QStringLiteral(":/icons/edit-copy.png"))), menu));
        menu->addAction(controller->createAction(
            "paste", tr("Paste"),
            QIcon::fromTheme(QStringLiteral("edit-paste"), QIcon(QStringLiteral(":/icons/edit-paste.png"))), menu));
        menu->addSeparator();
        menu->addAction(controller->createAction(
            "sel_all", tr("Select All"),
            QIcon::fromTheme(QStringLiteral("edit-select-all"), QIcon(QStringLiteral(":/icons/edit-select-all.png"))),
            menu));
        formatAction->setIcon(
            QIcon::fromTheme(QStringLiteral("run-build-clean"), QIcon::fromTheme(QStringLiteral("run-build-clean"))));
    }
    ...
    if (color.isValid())
    {
        if (isEnabled)
        {
            return QStringLiteral("QPushButton {color: %1; background-color: %2; }")
                .arg(color.lightness() > 127 ? QLatin1String("black") : QLatin1String("white"), color.name());
        }
...
QColor dlgTriggerEditor::parseButtonStyleSheetColors(const QString &styleSheetText, const bool isToGetForeground)
{
    ...
    {
        hexColorRegex.setPattern(
            QLatin1String("(?:[{ ])color:\\s*(?:#)([[:xdigit:]]{3,12})\\s*;")); // Capture group 1 is a foreground color
                                                                                // made of hex digits
        ...
        if (match.hasMatch())
        {
            ...
            default:
                ...
                qDebug().noquote().nospace()
                    << "dlgTriggerEditor::parseButtonStyleSheetColors(\"" << styleSheetText << "\", "
                    << isToGetForeground << ") ERROR - Invalid hex string as foreground color!";
                return QColor();
            }
        }
        else
        {
            namedColorRegex.setPattern(
                QLatin1String("(?:[{ ])color:\\s*(\\w{3,})\\s*;")); // Capture group 1 is a word for a foreground color
            ...
            if (match.hasMatch())
            {
                ...
                {
                    qDebug().noquote().nospace() << "dlgTriggerEditor::parseButtonStyleSheetColors(\"" << styleSheetText
                                                 << "\", " << isToGetForeground << ") ERROR - Invaid string \""
            ...
            else
            {
                qDebug().noquote().nospace()
                    << "dlgTriggerEditor::parseButtonStyleSheetColors(\"" << styleSheetText << "\", "
                    << isToGetForeground << ") ERROR - No string as name of foreground color found!";
                return QColor();
            }
        }
    }
    ...
    {
        hexColorRegex.setPattern(QLatin1String(
            "(?:[{ ])background-color:\\s*(?:#)([[:xdigit:]]{3,12})\\s*;")); // Capture group 1 is a background color
                                                                             // made of hex digits
        QRegularExpressionMatch match = hexColorRegex.match(styleSheetText);
        if (match.hasMatch())
        {
            ...
            default:
                ...
                qDebug().noquote().nospace()
                    << "dlgTriggerEditor::parseButtonStyleSheetColors(\"" << styleSheetText << "\", "
                    << isToGetForeground << ") ERROR - Invalid hex string as background color!";
                return QColor();
            }
        }
        else
        {
            namedColorRegex.setPattern(QLatin1String(
                "(?:[{ ])background-color:\\s*(\\w{3,})\\s*;")); // Capture group 1 is a word for a background color
            match = namedColorRegex.match(styleSheetText);
            if (match.hasMatch())
            {
                if (QColor::isValidColor(match.captured(1)))
            ...
            else
            {
                qDebug().noquote().nospace()
                    << "dlgTriggerEditor::parseButtonStyleSheetColors(\"" << styleSheetText << "\", "
                    << isToGetForeground << ") ERROR - No string as name of background color found!";
                return QColor();
            }
        }
    }
...
void dlgTriggerEditor::slot_toggleGroupBoxColorizeTrigger(const bool state)
{
    ...
    if (state)
    {
        ...
        mpTriggersMainArea->pushButtonFgColor->setStyleSheet(generateButtonStyleSheet(
            mpTriggersMainArea->pushButtonFgColor->property(cButtonBaseColor).toString(), true));
        mpTriggersMainArea->pushButtonBgColor->setStyleSheet(generateButtonStyleSheet(
            mpTriggersMainArea->pushButtonBgColor->property(cButtonBaseColor).toString(), true));
    }
    else
    {
        // Disabled so make buttons greyed out a bit:
        mpTriggersMainArea->pushButtonFgColor->setStyleSheet(generateButtonStyleSheet(
            mpTriggersMainArea->pushButtonFgColor->property(cButtonBaseColor).toString(), false));
        mpTriggersMainArea->pushButtonBgColor->setStyleSheet(generateButtonStyleSheet(
            mpTriggersMainArea->pushButtonBgColor->property(cButtonBaseColor).toString(), false));
    }
}
...
void dlgTriggerEditor::slot_rightSplitterMoved(const int, const int)
{
    ...
    if (mpTriggersMainArea->isVisible())
    {
        ...
        if (mpTriggersMainArea->toolButton_toggleExtraControls->isChecked())
        {
            ...
            if ((mpTriggersMainArea->widget_left->height()) <=
                    (mpTriggersMainArea->widget_right->minimumSizeHint().height() + hysteresis) ||
        ...
        else
        {
            ...
            if ((mpTriggersMainArea->widget_left->height() - bottomWidgetHeight) >
                mpTriggersMainArea->widget_right->minimumSizeHint().height() - hysteresis)
            {
