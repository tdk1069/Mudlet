...
dlgPackageExporter::dlgPackageExporter(QWidget *parent, Host *pHost)
    : QDialog(parent), ui(new Ui::dlgPackageExporter), mpHost(pHost)
{
    ui->setupUi(this);
    ...
    mExportButton->setText(
        tr("Export", "Text for button to perform the package export on the items the user has selected."));
    ...
    mPackagePath = QFileDialog::getExistingDirectory(nullptr, tr("Where do you want to save the package?"),
                                                     mudlet::getMudletPath(mudlet::profileHomePath, profileName),
    ...
    mStagingDirName =
        mudlet::getMudletPath(mudlet::profileDataItemPath, profileName, QStringLiteral("tmp/%1").arg(mPackageName));
...
bool dlgPackageExporter::writeFileToZip(const QString &archiveFileName, const QString &fileSystemFileName, zip *archive)
{
    ...
    if (s == nullptr)
    {
        displayResultMessage(tr("Failed to open file \"%1\" to place into package. Error message was: \"%2\".",
                                ...
                                "This error message will appear when a file is to be placed into the package but the "
                                "code cannot open it.")
                                 .arg(fileSystemFileName, QString::fromUtf8(zip_strerror(archive))),
                             false);
        return false;
    }
    ...
    if (zip_file_add(archive, archiveFileName.toUtf8().constData(), s, ZIP_FL_ENC_UTF_8 | ZIP_FL_OVERWRITE) == -1)
    {
        displayResultMessage(tr("Failed to add file \"%1\" to package \"%2\". Error message was: \"%3\".",
                                // Intentional comment to separate arguments
                                "This error message will appear when a file is to be placed into the package but "
                                "cannot be done for some reason.")
                                 .arg(archiveFileName, mPackagePathFileName, QString::fromUtf8(zip_strerror(archive))),
                             false);
        return false;
    }
...
void dlgPackageExporter::slot_export_package()
{
    ...
    for (auto item : qAsConst(trigList))
    {
        ...
        else if (item->checkState(0) == Qt::Checked && triggerMap.contains(item) &&
                 triggerMap[item]->mModuleMasterFolder)
        {
    ...
    if (!writer.exportPackage(mXmlPathFileName))
    {
        displayResultMessage(tr("Failed to export. Could not write Mudlet items to the file \"%1\".",
                                // Intentional comment to separate arguments
                                "This error message is shown when all the Mudlet items cannot be written to the "
                                "'packageName'.xml file in the base directory of the place where all the files are "
                                "staged before being compressed into the package file. The full path and filename are "
                                "shown in %1 to help the user diagnose what might have happened.")
                                 .arg(mXmlPathFileName),
                             false);
        ...
        if (!archive)
        {
            ...
            displayResultMessage(
                tr("Failed to open package file. Error is: \"%1\".",
                   // Intentional comment to separate arguments
                   "This error message is shown when the libzip library code is unable to open the file that was to be "
                   "the end result of the export process. As this may be an existing file anywhere in the computer's "
                   "file-system(s) it is possible that permissions on the directory or an existing file that is to be "
                   "overwritten may be a source of problems here.")
 ...
 */
            ...
            QDirIterator itDir(mStagingDirName, QDir::NoDotAndDotDot | QDir::AllDirs | QDir::Files,
            ...
            while (itDir.hasNext() && isOk)
            {
                ...
                //              qDebug() << " parsing entry:" << itEntry << " fileName() is:" << itDir.fileName() << "
                //              filePath() is:" << itDir.filePath();
                ...
                {
                    qWarning() << "dlgPackageExporter::slot_export_package() skipping file: " << itDir.fileName()
                               << "it is NOT readable!";
                    continue;
                }
            ...
            while (itDirectoryName.hasNext() && isOk)
            {
                ...
                if (zip_dir_add(archive, directoryName.toStdString().c_str(), ZIP_FL_ENC_UTF_8) == -1)
                {
                    displayResultMessage(tr("Failed to add directory \"%1\" to package. Error is: \"%2\".")
                                             .arg(directoryName, QString::fromUtf8(zip_strerror(archive))),
                                         false);
                    zip_close(archive);
                    isOk = false;
                }
            ...
            if (isOk)
            {
                ...
                {
                    if (!writeFileToZip(QStringLiteral("config.lua"), fileEntries.value(QStringLiteral("config.lua")),
                                        archive))
                    {
                        /* isOk = false; */
                    }
                    else
                    {
                     ...
                     displayResultMessage(tr("Required \"config.lua\" file not found to include in the package. Did you
                 remove or rename it?"), false); zip_close(archive); isOk = false;
                 }*/
            }
                ...
                else
                {
                    displayResultMessage(tr("Required file \"%1\" was not found in the staging area. "
                                            ...
                                            "Do you have the necessary permissions and free disk-space?")
                                             .arg(mXmlPathFileName, QDir(mStagingDirName).canonicalPath()),
                                         false);
                    isOk = false;
                }
            }
            ...
            if (isOk)
            {
                ...
                if (ze)
                {
                    displayResultMessage(tr("Failed to write files into and then close the package. Error is: \"%1\".",
                                            // Intentional comment to separate arguments
                                            "This error message is displayed at the final stage of exporting a package "
                                            "when all the sourced files are finally put into the archive. "
                                            "Unfortunately this may be the point at which something breaks because a "
                                            "problem was not spotted/detected in the process earlier...")
    ...
    if (isOk)
    {
        ...
        displayResultMessage(tr("Package \"%1\" exported to: %2")
                                 ...
                                 .arg(mPackagePath.toHtmlEscaped(), mPackagePath.toHtmlEscaped()),
                             true);
...
void dlgPackageExporter::displayResultMessage(const QString &html, bool const isSuccessMessage)
{
    ...
    ui->infoLabel->setText(
        QStringLiteral("<p><b><big>%1</big><b></p>"
                       "<p>%2</p>")
            .arg(html, tr("Why not <a href=\"https://forums.mudlet.org/viewforum.php?f=6\">upload</a> your package for "
                          "other Mudlet users?",
                          // Intentional comment to separate arguments
