...
TRoomDB::TRoomDB(TMap *pMap)
    : mpMap(pMap), mpTempRoomDeletionSet(nullptr), mUnnamedAreaName(tr("Unnamed Area")),
...
void TRoomDB::deleteValuesFromEntranceMap(QSet<int> &valueSet)
{
    ...
    qDebug() << "TRoomDB::deleteValuesFromEntranceMap() with a list of:" << valueSet.size()
             << "items, run time:" << timer.nsecsElapsed() * 1.0e-9 << "sec.";
}
...
void TRoomDB::updateEntranceMap(TRoom *pR, bool isMapLoading)
{
    ...
    if (pR)
    {
        ...
        if (showDebug)
        {
            ...
            if (values.isEmpty())
            {
                qDebug() << "TRoomDB::updateEntranceMap(TRoom * pR) called for room with id:" << id
...
bool TRoomDB::__removeRoom(int id)
{
    ...
    if (pR)
    {
        if (mpTempRoomDeletionSet && mpTempRoomDeletionSet->size() > 1)
        { // We are deleting multiple rooms
            if (!isBulkDelete)
            {
        ...
        { // We are deleting a single room
            if (isBulkDelete)
            { // Last time was a bulk delete but it isn't one now
                isBulkDelete = false;
            }
            _entranceMap.clear();
        ...
        {
            if (i.value() == id || (mpTempRoomDeletionSet && mpTempRoomDeletionSet->size() > 1 &&
                                    mpTempRoomDeletionSet->contains(i.value())))
            {
                ++i;
        ...
        if ((!mpTempRoomDeletionSet) || mpTempRoomDeletionSet->size() == 1)
        {                                    // if NOT deleting multiple rooms
...
void TRoomDB::removeRoom(QSet<int> &ids)
{
    ...
    qDebug() << "TRoomDB::removeRoom(QList<int>) run time for" << roomcount << "rooms:" << timer.nsecsElapsed() * 1.0e-9
             << "sec.";
}
...
bool TRoomDB::setAreaName(int areaID, QString name)
{
    if (areaID < 1)
    {
        qWarning() << "TRoomDB::setAreaName((int)areaID, (QString)name): WARNING: Suspect area id:" << areaID
                   << "supplied.";
        return false;
    }
    if (name.isEmpty())
    ...
    else if (areaNamesMap.values().count(name) > 0)
    {
        ...
        {
            qWarning() << "TRoomDB::setAreaName((int)areaID, (QString)name): WARNING: Duplicate name supplied" << name
    ...
    if (!areas.contains(id))
    {
        ...
        if (!areaNamesMap.contains(id))
        {
            ...
            if (areaNamesMap.values().contains(newAreaName))
            {
                ...
                {
                    newAreaName =
                        QStringLiteral("%1_%2").arg(mUnnamedAreaName).arg(++deduplicateSuffix, 3, 10, QLatin1Char('0'));
...
void TRoomDB::auditRooms(QHash<int, int> &roomRemapping, QHash<int, int> &areaRemapping)
{
    ...
    while (itUsedArea.hasNext())
    {
        ...
        if (!areas.contains(usedAreaId))
        {
            ...
            {
                QString warnMsg =
                    tr("[ ALERT ] - Area with id: %1 expected but not found, will be created.").arg(usedAreaId);
                mpMap->postMessage(warnMsg);
            }
            mpMap->appendAreaErrorMsg(
                usedAreaId, tr("[ ALERT ] - Area with this id expected but not found, will be created."), true);
    ...
    if (!missingAreasNeeded.isEmpty())
    {
        ...
        {
            QString alertMsg =
                tr("[ ALERT ] - %n area(s) detected as missing in map: adding it/them in.\n"
                   " Look for further messsages related to the rooms that are supposed\n"
                   " to be in this/these area(s)...",
                   "Making use of %n to allow quantity dependent message form 8-) !", missingAreasNeeded.count());
    ...
    if (!areaRemapping.isEmpty())
    {
        ...
        mpMap->appendErrorMsg(
            tr("[ ALERT ] - Bad, (less than +1 and not the reserved -1) area ids found (count: %1) in map!"
        ...
        while (itRemappedArea.hasNext())
        {
            ...
            if (mudlet::self()->showMapAuditErrors())
            {
                infoMsg.append(QStringLiteral("\n%1 ==> %2")
                                   .arg(QString::number(faultyAreaId), QString::number(replacementAreaId)));
            }

            mpMap->appendAreaErrorMsg(
                faultyAreaId, tr("[ INFO ]  - The area with this bad id was renumbered to: %1.").arg(replacementAreaId),
                true);
            mpMap->appendAreaErrorMsg(replacementAreaId,
                                      tr("[ INFO ]  - This area was renumbered from the bad id: %1.").arg(faultyAreaId),
                                      true);
            ...
            else
            {
                ...
                if (areaNamesMap.values().contains(newAreaName))
                {
                    ...
                    do
                    {
                        newAreaName = QStringLiteral("%1_%2")
                                          .arg(mUnnamedAreaName)
                                          .arg(++deduplicateSuffix, 3, 10, QLatin1Char('0'));
                    } while (areaNamesMap.values().contains(newAreaName));
                }
    ...
    if (!roomRemapping.isEmpty())
    {
        ...
        while (itRenumberedRoomId.hasNext())
        {
            ...
            if (mudlet::self()->showMapAuditErrors())
            {
                infoMsg.append(
                    QStringLiteral("%1 ==> %2")
                        .arg(QString::number(itRenumberedRoomId.key()), QString::number(itRenumberedRoomId.value())));
            }

            mpMap->appendRoomErrorMsg(
                itRenumberedRoomId.key(),
                tr("[ INFO ]  - This room with the bad id was renumbered to: %1.").arg(itRenumberedRoomId.value()),
                true);
            mpMap->appendRoomErrorMsg(
                itRenumberedRoomId.value(),
                tr("[ INFO ]  - This room was renumbered from the bad id: %1.").arg(itRenumberedRoomId.key()), true);
        }
        if (mudlet::self()->showMapAuditErrors())
        {
            mpMap->postMessage(infoMsg);
    ...
    { // Block the following code to limit the scope of the itRoom iterator
        ...
        while (itRoom.hasNext())
        {
            ...
            if (_set.count() < _listCount)
            {
                ...
                {
                    QString infoMsg =
                        tr("[ INFO ]  - Duplicate exit stub identifiers found in room id: %1, this is an\n"
                           "anomaly but has been cleaned up easily.")
                            .arg(itRoom.key());
                    mpMap->postMessage(infoMsg);
                }
                mpMap->appendRoomErrorMsg(itRoom.key(),
                                          tr("[ INFO ]  - Duplicate exit stub identifiers found in room, this is an "
                                             "anomaly but has been cleaned up easily."),
                                          false);
            }
            ...
            if (_set.count() < _listCount)
            {
                ...
                {
                    QString infoMsg =
                        tr("[ INFO ]  - Duplicate exit lock identifiers found in room id: %1, this is an\n"
                           "anomaly but has been cleaned up easily.")
                            .arg(itRoom.key());
                    mpMap->postMessage(infoMsg);
                }
                mpMap->appendRoomErrorMsg(itRoom.key(),
                                          tr("[ INFO ]  - Duplicate exit lock identifiers found in room, this is an "
                                             "anomaly but has been cleaned up easily."),
                                          false);
            }
        ...
        while (itArea.hasNext())
        {
            ...
            if (!missingRooms.isEmpty())
            {
                ...
                mpMap->appendAreaErrorMsg(itArea.key(),
                                          tr("[ INFO ]  - In this area there were %1 rooms missing from those it "
                                             "should be recorded as possessing."
                                             "  They are: %2."
                                             "  They have been added.")
                                              .arg(missingRooms.count())
                                              .arg(roomList.join(QStringLiteral(", "))),
            ...
            if (!extraRooms.isEmpty())
            {
                ...
                {
                    QString infoMsg =
                        tr("[ INFO ]  - In area with id: %1 there were %2 extra rooms compared to those it\n"
                ...
                mpMap->appendAreaErrorMsg(itArea.key(),
                                          tr("[ INFO ]  - In this area there were %1 extra rooms that it should not be "
...
void TRoomDB::restoreAreaMap(QDataStream &ifs)
{
    ...
    while (itArea.hasNext())
    {
        ...
        if (areaNamesMap.values().contains(nonEmptyAreaName))
        {
            ...
            {
                replacementName =
                    QStringLiteral("%1_%2").arg(nonEmptyAreaName).arg(++deduplicateSuffix, 3, 10, QLatin1Char('0'));
    ...
    if (!renamedMap.empty() || isEmptyAreaNamePresent)
    {
        ...
        if (!renamedMap.empty())
        {
            ...
            while (itRemappedNames.hasPrevious())
            {
                ...
                mpMap->appendAreaErrorMsg(
                    areaNamesMap.key(itRemappedNames.value()),
                    tr(R"([ INFO ]  - Area name changed to prevent duplicates or unnamed ones; old name: "%1", new name: "%2".)")
                        .arg(oldName, itRemappedNames.value()),
                    true);
                ;
            }
    ...
    if (!areaNamesMap.contains(-1))
    {
        ...
        QString defaultAreaNameInsertionMsg =
            tr("[ INFO ]  - Default (reset) area name (for rooms that have not been assigned to an\n"
