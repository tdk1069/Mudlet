...
XMLimport::XMLimport(Host *pH)
    : mpHost(pH), mPackageName(QString()), mpTrigger(Q_NULLPTR), mpTimer(Q_NULLPTR), mpAlias(Q_NULLPTR),
      mpKey(Q_NULLPTR), mpAction(Q_NULLPTR), mpScript(Q_NULLPTR), mpVar(Q_NULLPTR), gotTrigger(false), gotTimer(false),
      gotAlias(false), gotKey(false), gotAction(false), gotScript(false), module(0), mMaxRoomId(0), mMaxAreaId(-1),
...
bool XMLimport::importPackage(QFile *pfile, QString packName, int moduleFlag, QString *pVersionString)
{
        ...
        if (isStartElement())
        {
            if (name() == QStringLiteral("MudletPackage"))
            {
                    ...
                    /*||(mVersionMajor==1&&mVersionMinor)*/)
                {
                    ...
                    QString moanMsg =
                        tr("[ ALERT ] - Sorry, the file being read:\n"
                           "\"%1\"\n"
    ...
    if (!packName.isEmpty())
    {
        ...
        if (gotTimer)
        { // packName is NOT empty for modules...!
            mpTimer->setIsActive(true);
...
void XMLimport::readHostPackage(Host *pHost)
{
    ...
    if (attributes().hasAttribute("logFileNameFormat"))
    {
        ...
        pHost->mLogFileNameFormat = attributes()
                                        .value("logFileNameFormat")
                                        .toString()
                                        .replace(QLatin1String("hh"), QLatin1String("HH"), Qt::CaseSensitive);
    ...
    {
        pHost->mTimerDebugOutputSuppressionInterval = QTime::fromString(
            attributes().value(QLatin1String("mTimerSupressionInterval")).toString(), QLatin1String("hh:mm:ss.zzz"));
    }
    else
    {
    ...
    {
        pHost->mDiscordAccessFlags =
            static_cast<Host::DiscordOptionFlags>(attributes().value("mDiscordAccessFlags").toString().toInt());
    }
    ...
    {
        pHost->mRequiredDiscordUserDiscriminator =
            attributes().value(QLatin1String("mRequiredDiscordUserDiscriminator")).toString();
    }
    else
    {
    ...
    {
        pHost->setHaveColorSpaceId(attributes().value(QLatin1String("mSGRCodeHasColSpaceId")).toString() ==
                                   QLatin1String("yes"));
    }
    else
    {
    ...
    {
        pHost->setMayRedefineColors(attributes().value(QLatin1String("mServerMayRedefineColors")).toString() ==
                                    QLatin1String("yes"));
    }
    else
    {
    ...
    if (attributes().hasAttribute(QLatin1String("playerRoomStyle")))
    {
        ...
        outerDiameterPercentage =
            static_cast<quint8>(qBound(0, attributes().value(QLatin1String("playerRoomOuterDiameter")).toInt(), 255));
        innerDiameterPercentage =
            static_cast<quint8>(qBound(0, attributes().value(QLatin1String("playerRoomInnerDiameter")).toInt(), 255));
        ...
        pHost->setPlayerRoomStyleDetails(styleCode, outerDiameterPercentage, innerDiameterPercentage, outerColor,
                                         innerColor);
        if (pHost->mpMap)
        {
    ...
    {
        pHost->setSearchOptions(
            static_cast<dlgTriggerEditor::SearchOptions>(attributes().value("EditorSearchOptions").toInt()));
    }
    ...
    while (!atEnd())
    {
        ...
        else if (isStartElement())
        {
            ...
            else if (name() == "script")
            {
                ...
                {
                    qDebug().nospace()
                        << "XMLimport::readTriggerGroup(...): ERROR: can not compile trigger's lua code for: "
                        << pT->getName();
                }
            }
            ...
            else if (name() == "regexCodePropertyList")
            {
                ...
                if (Q_UNLIKELY(pT->mRegexCodeList.count() != pT->mRegexCodePropertyList.count()))
                {
                    qWarning().nospace() << "XMLimport::readTriggerGroup(...) ERROR: "
                                         ...
                                         << pT->getName() << " there were " << pT->mRegexCodeList.count()
                                         << " 'regexCodeList' sub-elements and " << pT->mRegexCodePropertyList.count()
    ...
    while (!atEnd())
    {
        ...
        else if (isStartElement())
        {
            ...
            else if (name() == "script")
            {
                ...
                {
                    qDebug().nospace()
                        << "XMLimport::readTimerGroup(...): ERROR: can not compile timer's lua code for: "
                        << pT->getName();
                }
            }
    ...
    while (!atEnd())
    {
        ...
        else if (isStartElement())
        {
            ...
            else if (name() == "script")
            {
                ...
                {
                    qDebug().nospace()
                        << "XMLimport::readAliasGroup(...): ERROR: can not compile alias's lua code for: "
                        << pT->getName();
                }
            }
            else if (name() == "command")
    ...
    while (!atEnd())
    {
        ...
        else if (isStartElement())
        {
            ...
            else if (name() == "script")
            {
                ...
                {
                    qDebug().nospace()
                        << "XMLimport::readActionGroup(...): ERROR: can not compile action's lua code for: "
                        << pT->getName();
                }
            }
            else if (name() == "css")
    ...
    while (!atEnd())
    {
        ...
        else if (isStartElement())
        {
            ...
            else if (name() == "script")
            {
                ...
                {
                    qDebug().nospace()
                        << "XMLimport::readScriptGroup(...): ERROR: can not compile script's lua code for: "
                        << pT->getName();
                }
            }
    ...
    while (!atEnd())
    {
        ...
        else if (isStartElement())
        {
            ...
            else if (name() == "script")
            {
                ...
                if (!pT->setScript(tempScript))
                {
                    qDebug().nospace() << "XMLimport::readKeyGroup(...): ERROR: can not compile key's lua code for: "
                                       << pT->getName();
                }
            }
            else if (name() == "command")
    ...
    while (itPattern.hasNext() && itType.hasNext())
    {
        if (itType.next() == REGEX_COLOR_PATTERN)
        {
            ...
            if (match.capturedTexts().size() == 3)
            {
                ...
                if (!isFgOk)
                {
                    qDebug() << "XMLimport::remapColorsToAnsiNumber(...) ERROR - failed to extract FG color code from "
                                "pattern text:"
                ...
                if (!isBgOk)
                {
                    qDebug() << "XMLimport::remapColorsToAnsiNumber(...) ERROR - failed to extract BG color code from "
                                "pattern text:"
        ...
        else if (isStartElement())
        {
            if (name() == "stopwatch")
            {
                ...
                if (attributes().value("running") == "yes")
                {
                    ...
                    pStopWatch->mEffectiveStartDateTime.setMSecsSinceEpoch(
                        attributes().value("effectiveStartDateTimeEpochMSecs").toLongLong());
                }
                else
                {
