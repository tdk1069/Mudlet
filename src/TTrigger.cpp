...
TTrigger::TTrigger(TTrigger *parent, Host *pHost)
    : Tree<TTrigger>(parent), mTriggerContainsPerlRegex(false), mPerlSlashGOption(false), mFilterTrigger(false),
      mSoundTrigger(false), mStayOpen(0), mColorTrigger(false), mKeepFiring(0), mpHost(pHost), exportItem(true),
      mModuleMasterFolder(false), mNeedsToBeCompiled(true), mTriggerType(REGEX_SUBSTRING), mIsLineTrigger(false),
      mStartOfLineDelta(0), mLineDelta(3), mIsMultiline(false), mConditionLineDelta(0),
      mpLua(mpHost->getLuaInterpreter()), mFgColor(QColor(Qt::red)), mBgColor(QColor(Qt::yellow)),
      mIsColorizerTrigger(false), mModuleMember(false), mColorTriggerFgAnsi(scmIgnored),
      mColorTriggerBgAnsi(scmIgnored), mRegisteredAnonymousLuaFunction(false), mExpiryCount(-1)
{
}

TTrigger::TTrigger(const QString &name, const QStringList &regexList, const QList<int> &regexProperyList,
                   bool isMultiline, Host *pHost)
    : Tree<TTrigger>(nullptr), mTriggerContainsPerlRegex(false), mPerlSlashGOption(false), mFilterTrigger(false),
      mSoundTrigger(false), mStayOpen(0), mColorTrigger(false), mKeepFiring(0), mpHost(pHost), mName(name),
      mRegexCodeList(regexList), exportItem(true), mModuleMasterFolder(false), mRegexCodePropertyList(regexProperyList),
      mNeedsToBeCompiled(true), mTriggerType(REGEX_SUBSTRING), mIsLineTrigger(false), mStartOfLineDelta(0),
      mLineDelta(3), mIsMultiline(isMultiline), mConditionLineDelta(0), mpLua(mpHost->getLuaInterpreter()),
      mFgColor(QColor(Qt::red)), mBgColor(QColor(Qt::yellow)), mIsColorizerTrigger(false), mModuleMember(false),
      mColorTriggerFgAnsi(scmIgnored), mColorTriggerBgAnsi(scmIgnored), mRegisteredAnonymousLuaFunction(false),
      mExpiryCount(-1)
{
...
TTrigger::~TTrigger()
{
    ...
    while (itColorTable.hasNext())
    {
        ...
        {
            //            qDebug() << "TTrigger::~TTrigger() INFO: removing TColorTable from mColorPatternList:
            //            (ansiFg:"
...
bool TTrigger::setRegexCodeList(QStringList regexList, QList<int> propertyList)
{
    ...
    while (itColorTable.hasNext())
    {
        if (itColorTable.next())
        {
            //            qDebug() << "TTrigger::setRegexCodeList() INFO: removing TColorTable from mColorPatternList:
            //            (ansiFg:"
            //                     << itColorTable.peekPrevious()->ansiFg
            //                     << itColorTable.peekPrevious()->mFgColor
            //                     << "ansiBg:"
    ...
    if (propertyList.size() != regexList.size())
    {
        ...
        qDebug() << "[CRITICAL ERROR (plz report):] Trigger name=" << mName
                 << " aborting reason: propertyList.size() != regexList.size()";
    }
    ...
    for (int i = 0; i < regexList.size(); i++)
    {
        ...
        if (propertyList.at(i) == REGEX_PERL)
        {
            ...
            QSharedPointer<pcre> re(pcre_compile(regexp.constData(), PCRE_UTF8 | PCRE_UCP, &error, &erroffset, nullptr),
                                    pcre_deleter);
            ...
            if (!mpLua->compile(code, error, QString::fromStdString(funcName)))
            {
                ...
                if (mudlet::debugMode)
                {
                    ...
                    TDebug(QColor(Qt::red), QColor(Qt::gray))
                            << R"(in lua condition function: ")" << regexList.at(i) << "\"\n" >>
                        0;
                }
            }
            else
        ...
        if (propertyList[i] == REGEX_COLOR_PATTERN)
        {
            ...
            {
                setError(QStringLiteral("<b><font color='blue'>%1</font></b>")
                             .arg(tr("Error: in item %1, no colors to match were set - at least <i>one</i> of the "
                                     "foreground or background must not be <i>ignored</i>.")
...
bool TTrigger::match_perl(char *subject, const QString &toMatch, int regexNumber, int posOffset)
{
    ...
    if (!re)
    {
        if (mudlet::debugMode)
        {
            ...
            TDebug(QColor(Qt::darkRed), QColor(Qt::darkGray)) << " the regex of trigger " << mName
                                                              << " does not compile. Please correct the expression. "
                                                                 "This trigger will never match until it is fixed.\n" >>
                0;
        }
    ...
    {
        qDebug() << "CRITICAL ERROR: SHOULD NOT HAPPEN->pcre_info() got wrong num of cap groups ovector only has room "
                    "for %d captured substrings\n";
    }
    else
    {
        if (mudlet::debugMode)
        {
            TDebug(QColor(Qt::blue), QColor(Qt::black))
                    << "Trigger name=" << mName << "(" << mRegexCodeList.value(regexNumber) << ") matched.\n" >>
                0;
        }
    }
    ...
    for (; mPerlSlashGOption;)
    {
        ...
        {
            qDebug() << "CRITICAL ERROR: SHOULD NOT HAPPEN->pcre_info() got wrong num of cap groups ovector only has "
                        "room for %d captured substrings\n";
        }
        ...
        for (i = 0; i < rc; i++)
        {
            ...
            if (mudlet::debugMode)
            {
                TDebug(QColor(Qt::darkCyan), QColor(Qt::black))
                        << "<regex mode: match all> capture group #" << (i + 1) << " = " >>
                    0;
                TDebug(QColor(Qt::darkMagenta), QColor(Qt::black)) << "<" << match.c_str() << ">\n" >> 0;
            }
        }
...
bool TTrigger::match_begin_of_line_substring(const QString &toMatch, const QString &regex, int regexNumber,
                                             int posOffset)
{
...
inline void TTrigger::updateMultistates(int regexNumber, std::list<std::string> &captureList, std::list<int> &posList)
{
    if (regexNumber == 0)
    {
        ...
        if (mudlet::debugMode)
        {
            TDebug(QColor(Qt::darkYellow), QColor(Qt::black))
                    << "match state " << mConditionMap.size() << "/" << mConditionMap.size() << " condition #"
                    << regexNumber << "=true (" << regexNumber << "/" << mRegexCodeList.size()
                    << ") regex=" << mRegexCodeList[regexNumber] << "\n" >>
                0;
        }
    }
    else
    {
        ...
        for (auto &matchStatePair : mConditionMap)
        {
            ...
            if (matchStatePair.second->nextCondition() == regexNumber)
            {
                ...
                {
                    TDebug(QColor(Qt::darkYellow), QColor(Qt::black))
                            << "match state " << k << "/" << mConditionMap.size() << " condition #" << regexNumber
                            << "=true (" << regexNumber << "/" << mRegexCodeList.size()
                            << ") regex=" << mRegexCodeList[regexNumber] << "\n" >>
                        0;
                }
...
bool TTrigger::match_substring(const QString &toMatch, const QString &regex, int regexNumber, int posOffset)
{
    ...
    if (where != -1)
    {
        ...
        if (mudlet::debugMode)
        {
            TDebug(QColor(Qt::cyan), QColor(Qt::black))
                    << "Trigger name=" << mName << "(" << mRegexCodeList.value(regexNumber) << ") matched.\n" >>
                0;
        }
        if (mIsColorizerTrigger)
...
bool TTrigger::match_color_pattern(int line, int regexNumber)
{
    ...
    for (auto it = bufferLine.begin(); it != bufferLine.end(); ++it, ++pos)
    {
        ...
        if (((pCT->ansiFg == scmIgnored) ||
             ((pCT->ansiFg == scmDefault) && mpHost->mpConsole->mFgColor == (*it).foreground()) ||
             (pCT->mFgColor == (*it).foreground())) &&
            ((pCT->ansiBg == scmIgnored) ||
             ((pCT->ansiBg == scmDefault) && mpHost->mpConsole->mBgColor == (*it).background()) ||
             (pCT->mBgColor == (*it).background())))
        {
    ...
    if (canExecute)
    {
        if (mIsColorizerTrigger)
        {
            ...
            {
                int begin = *iti;
                qDebug() << "TTrigger::match_color_pattern(" << line << "," << regexNumber
                         << ") INFO - match found: " << (*its).c_str() << " size is:" << (*its).size();
                std::string &s = *its;
                int length = QString::fromStdString(s).size();
                pC->selectSection(begin, length);
                pC->setBgColor(r1, g1, b1);
        ...
        for (auto &matchStatePair : mConditionMap)
        {
            ...
            if (matchStatePair.second->nextCondition() == regexNumber)
            {
                if (matchStatePair.second->lineSpacerMatch(mRegexCodeList.value(regexNumber).toInt()))
                {
                    if (mudlet::debugMode)
                    {
                        TDebug(QColor(Qt::yellow), QColor(Qt::black))
                                << "Trigger name=" << mName << "(" << mRegexCodeList.value(regexNumber)
                                << ") condition #" << regexNumber << "=true " >>
                            0;
                        TDebug(QColor(Qt::darkYellow), QColor(Qt::black))
                                << "match state " << k << "/" << mConditionMap.size() << " condition #" << regexNumber
                                << "=true (" << regexNumber + 1 << "/" << mRegexCodeList.size()
                                << ") line spacer=" << mRegexCodeList.value(regexNumber) << "lines\n" >>
                            0;
                    }
                    matchStatePair.second->conditionMatched();
                    std::list<std::string> captureList;
...
bool TTrigger::match_prompt(int patternNumber)
{
    if (mpHost->mpConsole->mIsPromptLine)
    {
        ...
        {
            TDebug(QColor(Qt::yellow), QColor(Qt::black))
                    << "Trigger name=" << mName << "(" << mRegexCodeList.value(patternNumber) << ") matched.\n" >>
                0;
        }
        if (mIsMultiline)
        {
...
bool TTrigger::match(char *subject, const QString &toMatch, int line, int posOffset)
{
    ...
    if (isActive())
    {
        ...
        if (mIsMultiline)
        {
            ...
            for (auto &matchStatePair : mConditionMap)
            {
                ...
                if (matchStatePair.second->isComplete())
                {
                    ...
                    if (mudlet::debugMode)
                    {
                        TDebug(QColor(Qt::yellow), QColor(Qt::darkMagenta))
                                << "multiline trigger name=" << mName
                                << " *FIRES* all conditons are fullfilled. Executing script.\n" >>
                            0;
                    }
                    removeList.push_back(matchStatePair.first);
                    conditionMet = true;
                    TLuaInterpreter *pL = mpHost->getLuaInterpreter();
                    pL->setMultiCaptureGroups(matchStatePair.second->multiCaptureList,
                                              matchStatePair.second->multiCapturePosList);
                    execute();
                    pL->clearCaptureGroups();
                    if (mFilterTrigger)
                    {
            ...
            for (auto &matchState : removeList)
            {
                if (mConditionMap.find(matchState) != mConditionMap.end())
                {
                    ...
                    if (mudlet::debugMode)
                    {
                        TDebug(QColor(Qt::darkBlue), QColor(Qt::black))
        ...
        if (conditionMet && mExpiryCount > -1)
        {
            ...
            {
                TDebug(QColor(Qt::yellow), QColor(Qt::darkMagenta))
                        << tr("Trigger name=%1 will fire %n more time(s).\n", "", mExpiryCount).arg(mName) >>
                    0;
            }
        }
    ...
    if (!compileScript())
    {
        if (mudlet::debugMode)
        {
            TDebug(QColor(Qt::white), QColor(Qt::red))
                    << "ERROR: Lua compile error. compiling script of Trigger:" << mName << "\n" >>
                0;
        }
        mOK_code = false;
    }
...
void TTrigger::decodeColorPatternText(const QString &patternText, int &fgColorCode, int &bgColorCode)
{
    ...
    QRegularExpression regex =
        QRegularExpression(QStringLiteral("^ANSI_COLORS_F{(\\d+|DEFAULT|IGNORE)}_B{(\\d+|DEFAULT|IGNORE)}$"));
