...
stopWatch::stopWatch()
    : mIsInitialised(false), mIsRunning(false), mIsPersistent(false), mEffectiveStartDateTime(), mElapsedTime()
{
...
QString stopWatch::getElapsedDayTimeString() const
{
    ...
    return QStringLiteral("%1:%2:%3:%4:%5:%6")
        .arg((isNegative ? QLatin1String("-") : QLatin1String("+")), QString::number(days), QString::number(hours),
             QString::number(minutes), QString::number(seconds), QString::number(milliSeconds));
}

Host::Host(int port, const QString &hostname, const QString &login, const QString &pass, int id)
    : mTelnet(this, hostname), mpConsole(nullptr), mLuaInterpreter(this, hostname, id), commandLineMinimumHeight(30),
      mAlertOnNewData(true), mAllowToSendCommand(true), mAutoClearCommandLineAfterSend(false),
      mBlockScriptCompile(true), mBlockStopWatchCreation(true), mEchoLuaErrors(false), mBorderBottomHeight(0),
      mBorderLeftWidth(0), mBorderRightWidth(0), mBorderTopHeight(0),
      mCommandLineFont(QFont(QStringLiteral("Bitstream Vera Sans Mono"), 14, QFont::Normal)),
      mCommandSeparator(QStringLiteral(";;")),
      mDisplayFont(QFont(QStringLiteral("Bitstream Vera Sans Mono"), 14, QFont::Normal)), mEnableGMCP(true),
      mEnableMSSP(true), mEnableMSP(true), mEnableMSDP(false), mServerMXPenabled(true), mMediaLocationGMCP(QString()),
      mMediaLocationMSP(QString()), mFORCE_GA_OFF(false), mFORCE_NO_COMPRESSION(false), mFORCE_SAVE_ON_EXIT(false),
      mInsertedMissingLF(false), mSslTsl(false), mUseProxy(false), mProxyAddress(QString()), mProxyPort(0),
      mProxyUsername(QString()), mProxyPassword(QString()), mIsGoingDown(false), mIsProfileLoadingSequence(false),
      mLF_ON_GA(true), mNoAntiAlias(false), mpEditorDialog(nullptr), mpMap(new TMap(this, hostname)),
      mpMedia(new TMedia(this, hostname)), mpNotePad(nullptr), mPrintCommand(true), mIsRemoteEchoingActive(false),
      mIsCurrentLogFileInHtmlFormat(false), mIsNextLogFileInHtmlFormat(false), mIsLoggingTimestamps(false),
      mLogDir(QString()), mLogFileName(QString()),
      mLogFileNameFormat(QLatin1String(
          "yyyy-MM-dd#HH-mm-ss")) // In the past we have used "yyyy-MM-dd#hh-mm-ss" but we always want a 24-hour clock
      ,
      mResetProfile(false), mScreenHeight(25), mScreenWidth(90), mTimeout(60), mUSE_FORCE_LF_AFTER_PROMPT(false),
      mUSE_IRE_DRIVER_BUGFIX(true), mUSE_UNIX_EOL(false), mWrapAt(100), mWrapIndentCount(0), mEditorAutoComplete(true),
      mEditorTheme(QLatin1String("Mudlet")), mEditorThemeFile(QLatin1String("Mudlet.tmTheme")), mThemePreviewItemID(-1),
      mThemePreviewType(QString())
#if (QT_VERSION >= QT_VERSION_CHECK(5, 14, 0))
      ,
      mBlack(QColorConstants::Black), mLightBlack(QColorConstants::DarkGray), mRed(QColorConstants::DarkRed),
      mLightRed(QColorConstants::Red), mLightGreen(QColorConstants::Green), mGreen(QColorConstants::DarkGreen),
      mLightBlue(QColorConstants::Blue), mBlue(QColorConstants::DarkBlue), mLightYellow(QColorConstants::Yellow),
      mYellow(QColorConstants::DarkYellow), mLightCyan(QColorConstants::Cyan), mCyan(QColorConstants::DarkCyan),
      mLightMagenta(QColorConstants::Magenta), mMagenta(QColorConstants::DarkMagenta),
      mLightWhite(QColorConstants::White), mWhite(QColorConstants::LightGray), mFgColor(QColorConstants::LightGray),
      mBgColor(QColorConstants::Black), mCommandBgColor(QColorConstants::Black), mCommandFgColor(QColor(113, 113, 0)),
      mBlack_2(QColorConstants::Black), mLightBlack_2(QColorConstants::DarkGray), mRed_2(QColorConstants::DarkRed),
      mLightRed_2(QColorConstants::Red), mLightGreen_2(QColorConstants::Green), mGreen_2(QColorConstants::DarkGreen),
      mLightBlue_2(QColorConstants::Blue), mBlue_2(QColorConstants::DarkBlue), mLightYellow_2(QColorConstants::Yellow),
      mYellow_2(QColorConstants::DarkYellow), mLightCyan_2(QColorConstants::Cyan), mCyan_2(QColorConstants::DarkCyan),
      mLightMagenta_2(QColorConstants::Magenta), mMagenta_2(QColorConstants::DarkMagenta),
      mLightWhite_2(QColorConstants::White), mWhite_2(QColorConstants::LightGray),
      mFgColor_2(QColorConstants::LightGray), mBgColor_2(QColorConstants::Black)
#else
      ,
      mBlack(Qt::black), mLightBlack(Qt::darkGray), mRed(Qt::darkRed), mLightRed(Qt::red), mLightGreen(Qt::green),
      mGreen(Qt::darkGreen), mLightBlue(Qt::blue), mBlue(Qt::darkBlue), mLightYellow(Qt::yellow),
      mYellow(Qt::darkYellow), mLightCyan(Qt::cyan), mCyan(Qt::darkCyan), mLightMagenta(Qt::magenta),
      mMagenta(Qt::darkMagenta), mLightWhite(Qt::white), mWhite(Qt::lightGray), mFgColor(Qt::lightGray),
      mBgColor(Qt::black), mCommandBgColor(Qt::black), mCommandFgColor(QColor(113, 113, 0)), mBlack_2(Qt::black),
      mLightBlack_2(Qt::darkGray), mRed_2(Qt::darkRed), mLightRed_2(Qt::red), mLightGreen_2(Qt::green),
      mGreen_2(Qt::darkGreen), mLightBlue_2(Qt::blue), mBlue_2(Qt::darkBlue), mLightYellow_2(Qt::yellow),
      mYellow_2(Qt::darkYellow), mLightCyan_2(Qt::cyan), mCyan_2(Qt::darkCyan), mLightMagenta_2(Qt::magenta),
      mMagenta_2(Qt::darkMagenta), mLightWhite_2(Qt::white), mWhite_2(Qt::lightGray), mFgColor_2(Qt::lightGray),
      mBgColor_2(Qt::black)
#endif
      ,
      mMapStrongHighlight(false), mLogStatus(false), mEnableSpellCheck(true), mDiscordDisableServerSide(true),
      mDiscordAccessFlags(DiscordLuaAccessEnabled | DiscordSetSubMask), mLineSize(10.0), mRoomSize(0.5),
      mShowInfo(true), mBubbleMode(false), mShowRoomID(false), mShowPanel(true),
      mServerGUI_Package_version(QLatin1String("-1")), mServerGUI_Package_name(QLatin1String("nothing")),
      mAcceptServerGUI(true), mAcceptServerMedia(true), mCommandLineFgColor(Qt::darkGray),
      mCommandLineBgColor(Qt::black), mMapperUseAntiAlias(true), mFORCE_MXP_NEGOTIATION_OFF(false),
      mpDockableMapWidget(), mEnableTextAnalyzer(false), mTimerDebugOutputSuppressionInterval(QTime()),
      mTriggerUnit(this), mTimerUnit(this), mScriptUnit(this), mAliasUnit(this), mActionUnit(this), mKeyUnit(this),
      mHostID(id), mHostName(hostname), mIsClosingDown(false), mLogin(login), mPass(pass), mPort(port), mRetries(5),
      mSaveProfileOnExit(false), mHaveMapperScript(false), mAutoAmbigousWidthGlyphsSetting(true),
      mWideAmbigousWidthGlyphs(false), mSGRCodeHasColSpaceId(false), mServerMayRedefineColors(false),
      ...
      ,
      mEnableUserDictionary(true), mUseSharedDictionary(false), mPlayerRoomStyle(0), mPlayerRoomOuterColor(Qt::red),
      mPlayerRoomInnerColor(Qt::white), mPlayerRoomOuterDiameterPercentage(120), mPlayerRoomInnerDiameterPercentage(70),
      mProfileStyleSheet(QString()), mSearchOptions(dlgTriggerEditor::SearchOption::SearchOptionNone)
{
    ...
    mSearchEngineData = QMap<QString, QString>({{"Bing", "https://www.bing.com/search?q="},
...
void Host::saveModules(int sync, bool backup)
{
    ...
    while (it.hasNext())
    {
        ...
        if (backup)
        {
            ...
            savePathDir.rename(filename_xml, savePath + moduleName +
                                                 time); // move the old file, use the key (module name) as the file
        }
...
std::tuple<bool, QString, QString> Host::saveProfile(const QString &saveFolder, const QString &saveName,
                                                     bool syncModules)
{
    ...
    {
        filename_xml =
            QStringLiteral("%1/%2.xml")
                .arg(directory_xml, QDateTime::currentDateTime().toString(QStringLiteral("dd-MM-yyyy#hh-mm-ss")));
    }
    else
    {
...
void Host::thankForUsingPTB()
{
    ...
    postMessage(tr(R"([  OK  ]  - %1 Thanks a lot for using the Public Test Build!)", "%1 will be a random happy emoji")
                    .arg(randomIcon));
    postMessage(
        tr(R"([  OK  ]  - %1 Help us make Mudlet better by reporting any problems.)", "%1 will be a random happy emoji")
            .arg(randomIcon));
}
...
QPair<int, QString> Host::createStopWatch(const QString &name)
{
    ...
    if (!mStopWatchMap.isEmpty() && !name.isEmpty())
    {
        ...
        while (itStopWatch.hasNext())
        {
            ...
            {
                return qMakePair(0, QStringLiteral("a stopwatch with id %1 called \"%2\" already exists")
                                        .arg(QString::number(itStopWatch.key()), name));
            }
        }
    }
    int newWatchId = 1;
...
QPair<bool, QString> Host::startStopWatch(const QString &name)
{
    ...
    if (Q_LIKELY(pStopWatch))
    {
        ...
        return qMakePair(false, QStringLiteral("stopwatch with name \"%1\" (id:%2) was already running")
    ...
    Q_UNREACHABLE();
    return qMakePair(
        false, QStringLiteral("stopwatch with name \"%1\" (id:%2) not found").arg(name, QString::number(watchId)));
}
...
QPair<bool, QString> Host::stopStopWatch(const QString &name)
{
    ...
    if (Q_LIKELY(pStopWatch))
    {
        ...
        return qMakePair(false, QStringLiteral("stopwatch with name \"%1\" (id:%2) was already stopped")
                                    .arg(name, QString::number(watchId)));
    }
...
QPair<bool, QString> Host::resetStopWatch(const QString &name)
{
    ...
    if (Q_LIKELY(pStopWatch))
    {
        ...
        {
            return qMakePair(false, QStringLiteral("stopwatch with name \"%1\" (id:%2) was already reset")
                                        .arg(name, QString::number(watchId)));
        }
    }
    ...
    if (!newName.isEmpty())
    {
        ...
        while (itStopWatch.hasNext())
        {
            ...
            if (itStopWatch.value()->name() == newName)
            {
                ...
                {
                    return qMakePair(false,
                                     QStringLiteral("the name \"%1\" is already in use for another stopwatch (id:%2)")
                                         .arg(newName, QString::number(itStopWatch.key())));
                }
                else
                {
...
QPair<bool, QString> Host::setStopWatchName(const QString &currentName, const QString &newName)
{
    ...
    if (isAlreadyUsed)
    {
        return qMakePair(false, QStringLiteral("the name \"%1\" is already in use for another stopwatch (id:%2)")
                                    .arg(newName, QString::number(alreadyUsedId)));
    }
...
bool Host::installPackage(const QString &fileName, int module)
{
    ...
    //     This separation is necessary to be able to reuse code while avoiding infinite loops from script
    //     installations.
...
bool Host::uninstallPackage(const QString &packageName, int module)
{
    ...
    if (module)
    {
        ...
        // reinstall the package if it shared a module name.  This is a kludge, but it's cleaner than adding extra
        // arguments/etc imo
...
void Host::installPackageFonts(const QString &packageName)
{
    ...
    while (it.hasNext())
    {
        ...
        if (filePath.endsWith(QLatin1String(".otf"), Qt::CaseInsensitive) ||
            filePath.endsWith(QLatin1String(".ttf"), Qt::CaseInsensitive) ||
            filePath.endsWith(QLatin1String(".ttc"), Qt::CaseInsensitive) ||
            filePath.endsWith(QLatin1String(".otc"), Qt::CaseInsensitive))
        {
...
void Host::setWideAmbiguousEAsianGlyphs(const Qt::CheckState state)
{
    ...
    if (state == Qt::PartiallyChecked)
    {
        ...
        if (encoding == QLatin1String("GBK") || encoding == QLatin1String("GB18030") ||
            encoding == QLatin1String("Big5") || encoding == QLatin1String("Big5-HKSCS"))
        {
...
void Host::processGMCPDiscordInfo(const QJsonObject &discordInfo)
{
    ...
    if (inviteUrl != QJsonValue::Undefined && !inviteUrl.toString().isEmpty() &&
        inviteUrl.toString() != QStringLiteral("0"))
    {
        hasInvite = true;
    }
    ...
    if (gameName != QJsonValue::Undefined)
    {
        ...
        if (richPresenceSupported.first && pMudlet->mDiscord.usingMudletsDiscordID(this))
        {
            ...
            pMudlet->mDiscord.setLargeImageText(
                this, tr("%1 at %2:%3", "%1 is the game name and %2:%3 is game server address like: mudlet.org:23")
                          .arg(gameName.toString(), getUrl(), QString::number(getPort())));
        }
        else
        {
...
bool Host::discordUserIdMatch(const QString &userName, const QString &userDiscriminator) const
{
    ...
    if (!userDiscriminator.isEmpty() && !mRequiredDiscordUserDiscriminator.isEmpty() &&
...
// if necessary:
// DISABLED: - Prevent "None" option for user dictionary - modified to prevent original useDictionary argument from
// being false:
...
void Host::loadSecuredPassword()
{
    ...
    connect(job, &QKeychain::ReadPasswordJob::finished, this, [=](QKeychain::Job *job) {
        if (job->error())
        {
            ...
            {
                qDebug() << "Host::loadSecuredPassword ERROR: couldn't retrieve secure password for" << getName()
                         << ", error is:" << error;
            }
        }
        else
...
void Host::setPlayerRoomStyleDetails(const quint8 styleCode, const quint8 outerDiameter, const quint8 innerDiameter,
...
void Host::getPlayerRoomStyleDetails(quint8 &styleCode, quint8 &outerDiameter, quint8 &innerDiameter,
