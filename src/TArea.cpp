...
TArea::TArea(TMap *map, TRoomDB *pRDB)
    : min_x(0), min_y(0), min_z(0), max_x(0), max_y(0), max_z(0), gridMode(false), isZone(false), zoneAreaRef(0),
      mpRoomDB(pRDB), mIsDirty(false), mpMap(map)
{
}
...
void TArea::removeRoom(int room, bool isToDeferAreaRelatedRecalculations)
{
    ...
    if (runCount % 1000 == 0)
    {
        qDebug() << "TArea::removeRoom(" << room << ") from Area took" << thisTime * 1.0e-9
                 << "sec. this time and after" << runCount << "times the average is" << cumulativeMean << "sec.";
    }
}
...
const QMultiMap<int, QPair<QString, int>> TArea::getAreaExitRoomData() const
{
    ...
    while (itAreaExit.hasNext())
    {
        ...
        case DIR_OTHER:
            ...
            break;
        default:
            qWarning("TArea::getAreaExitRoomData() Warning: unrecognised exit code %i found for exit from room %i to "
                     "room %i.",
        ...
        if (pFromRoom)
        {
            ...
            while (itOtherExit.hasNext())
            {
                ...
                if (pToRoom && mpRoomDB->getArea(pToRoom->getArea()) != this)
                {
                    ...
                    if (itOtherExit.value().mid(0, 1) == QStringLiteral("0") ||
                        itOtherExit.value().mid(0, 1) == QStringLiteral("1"))
                    {
