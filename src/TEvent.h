...
inline QDebug &operator<<(QDebug &debug, const TEvent &event)
{
    ...
    while (i < argCount && i < typeCount)
    {
        ...
        {
            result.append(QLatin1String("[") % QString::number(i) % QLatin1String("{missing}") %
                          event.mArgumentList.at(i) % QLatin1String("]"));
        }
        else
        {
            ...
            else
            {
                ...
                {
                case ARGUMENT_TYPE_NUMBER:
                    result.append(QLatin1String("[") % QString::number(i) % QLatin1String("{number}") %
                                  QString::number(event.mArgumentList.at(i).toDouble()) % QLatin1String("]"));
                    break;
                case ARGUMENT_TYPE_BOOLEAN:
                    result.append(QLatin1String("[") % QString::number(i) % QLatin1String("{bool}") %
                                  (event.mArgumentList.at(i).toInt() ? QLatin1String("true") : QLatin1String("false")) %
                                  QLatin1String("]"));
                    break;
                case ARGUMENT_TYPE_NIL:
                    ...
                    break;
                default:
                    result.append(QLatin1String("[") % QString::number(i) % QLatin1String("{string}") %
                                  event.mArgumentList.at(i) % QLatin1String("]"));
                }
            }
            ++i;
