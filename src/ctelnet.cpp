...
cTelnet::cTelnet(Host *pH, const QString &profileName)
    : mResponseProcessed(true), networkLatency(), mAlertOnNewData(true), mGA_Driver(false), mFORCE_GA_OFF(false),
      mpComposer(nullptr), mpHost(pH), mEncoding(), mpPostingTimer(new QTimer(this)), mUSE_IRE_DRIVER_BUGFIX(false),
      mLF_ON_GA(false), mCommands(0), mMCCP_version_1(false), mMCCP_version_2(false), mpProgressDialog(),
      mProfileName(profileName), hostPort(), networkLatencyMin(), networkLatencyMax(), mWaitingForResponse(),
      mZstream(), recvdGA(), lastTimeOffset(), enableATCP(false), enableGMCP(false), enableMSSP(false),
      enableMSP(false), enableChannel102(false), mAutoReconnect(false), loadingReplay(false),
      mIsReplayRunFromLua(false), mEncodingWarningIssued(false), mEncoderFailureNoticeIssued(false),
      mConnectViaProxy(false)
{
    ...
    QTimer::singleShot(0, this, [this]() {
        ...
        connect(&socket, qOverload<const QList<QSslError> &>(&QSslSocket::sslErrors), this,
...
void cTelnet::encodingChanged(const QByteArray &requestedEncoding)
{
    ...
    QByteArray encoding =
        mAcceptableEncodings.contains("M_" + requestedEncoding) ? "M_" + requestedEncoding : requestedEncoding;
    if (mEncoding != encoding)
    {
        ...
        if (!mEncoding.isEmpty() && mEncoding != "ASCII")
        {
            ...
            {
                qDebug().nospace() << "cTelnet::encodingChanged(" << encoding
                                   << ") INFO - Installing a codec for OOB protocols that can handle: "
                                   << mpOutOfBandDataIncomingCodec->aliases();
            }
            else
            {
                qWarning().nospace() << "cTelnet::encodingChanged(" << encoding
                                     << ") WARNING - Unable to locate a codec for OOB protocols that can handle: "
                                     << mEncoding;
            }
        }
        else if (mpOutOfBandDataIncomingCodec)
        {
            ...
            qDebug().nospace() << "cTelnet::encodingChanged(" << encoding
                               << ") INFO - Uninstalling the codec for OOB protocols that can handle: "
                               << mpOutOfBandDataIncomingCodec->aliases()
                               << " as the new encoding setting of: " << encoding
...
QPair<bool, QString> cTelnet::setEncoding(const QByteArray &newEncoding, const bool saveValue)
{
    ...
    else if (!(mAcceptableEncodings.contains(newEncoding) || mAcceptableEncodings.contains("M_" + newEncoding)))
    {
        ...
        return qMakePair(false, QLatin1String(R"(Encoding ")") % newEncoding %
                                    QLatin1String("\" does not exist;\nuse one of the following:\n\"ASCII\", \"") %
                                    QLatin1String(fixedUpEncodings.join(R"(", ")")) % QLatin1String(R"(".)"));
    }
...
void cTelnet::handle_socket_signal_disconnected()
{
    ...
    if (!mpHost->mIsGoingDown)
    {
        ...
        else
        {
            ...
            if (reason ==
                QStringLiteral(
                    "Error during SSL handshake: error:140770FC:SSL routines:SSL23_GET_SERVER_HELLO:unknown protocol"))
            {
                reason =
                    tr("Secure connections aren't supported by this game on this port - try turning the option off.");
            }
            QString err = tr("[ ALERT ] - Socket got disconnected.\nReason: ") % reason;
            postMessage(err);
        }
    ...
    if (mpHost->mAllowToSendCommand)
    {
        ...
        if (!mEncoding.isEmpty())
        {
            if (outgoingDataEncoder)
            {
                ...
                {
                    QString errorMsg = tr(errorMsgTemplate, "%1 is the name of the encoding currently set.")
...
void cTelnet::processTelnetCommand(const std::string &command)
{
    ...
    case TN_WILL: {
        ...
        if (option == OPT_GMCP)
        {
            ...
            // APP_BUILD could, conceivably contain a non-ASCII character:
            output +=
                encodeAndCookBytes(R"(Core.Hello { "client": "Mudlet", "version": ")" APP_VERSION APP_BUILD R"("})");
            output += TN_IAC;
            output += TN_SE;
            socketOutRaw(output);
            ...
            output += OPT_GMCP;
            output +=
                R"(Core.Supports.Set [ "Char 1", "Char.Skills 1", "Char.Items 1", "Room 1", "IRE.Rift 1", "IRE.Composer 1", "External.Discord 1", "Client.Media 1"])";
            output += TN_IAC;
            output += TN_SE;
            socketOutRaw(output);
            ...
            if (!hisOptionState[idxOption])
            {
                ...
                else if ((option == OPT_COMPRESS) || (option == OPT_COMPRESS2))
                {
                    ...
                    if (mpHost->mFORCE_NO_COMPRESSION ||
                        ((option == OPT_COMPRESS) && (hisOptionState[static_cast<int>(OPT_COMPRESS2)])))
                    {
                        // protocol says: reject MCCP v1 if you have previously accepted MCCP v2...
                        sendTelnetOption(TN_DONT, option);
                        hisOptionState[idxOption] = false;
                        qDebug() << "Rejecting MCCP v1, because v2 has already been negotiated or FORCE COMPRESSION "
                                    "OFF is set to ON.";
                    }
                    else
                    {
    ...
    case TN_WONT: {
        ...
        else
        {
            ...
            if (hisOptionState[idxOption] || (heAnnouncedState[idxOption]))
            {
                ...
                {
                    mpHost->mIsRemoteEchoingActive = false;
                    qDebug() << "Server is stopping the ECHOing our output - so back to normal after, perhaps, sending "
                                "a password...";
                }
    ...
    case TN_SB: {
        ...
        if (option == OPT_ATCP)
        {
            ...
            else if (payload.startsWith(QByteArray("Client.GUI")))
            {
                ...
                if (mpHost->mServerGUI_Package_version != version)
                {
                    postMessage(tr("[ INFO ]  - The server wants to upgrade the GUI to new version '%1'.\n"
                                   "Uninstalling old version '%2'.")
                                    .arg(version, mpHost->mServerGUI_Package_version != QStringLiteral("-1")
                                                      ? mpHost->mServerGUI_Package_version
                                                      : QStringLiteral("(unknown)")));
                    // uninstall by previous known package name or current if we don't
                    // know it (in case of manual installation)
                    mpHost->uninstallPackage(mpHost->mServerGUI_Package_name != QStringLiteral("nothing")
                                                 ? mpHost->mServerGUI_Package_name
                                                 : packageName,
                                             0);
                ...
                postMessage(
                    tr("[ INFO ]  - Server offers downloadable GUI (url='%1') (package='%2').").arg(url, packageName));
                ...
                mpProgressDialog = new QProgressDialog(tr("downloading game GUI from server"),
                                                       tr("Cancel", "Cancel download of GUI package from Server"), 0,
                                                       4000000, mpHost->mpConsole);
        ...
        // Original fix by CR, second revision by MH - To take out normal MCCP version 1 option and 2, no need for this.
        // -MH
        // TODO: Remove these comments. Old boolean taken out for MCCP, and other options which were un-needed code.
        // Rev.3 -MH //
        ...
        case OPT_STATUS: {
            if (command.length() >= 6 && command[3] == TNSB_SEND && command[4] == TN_IAC && command[5] == TN_SE)
            {
                ...
                // FIXME: This is damaged at the moment as we do not properly take care of the bits for the protocols
                // that we manage ourselves e.g. ATCP/GMCP/MSDP/MXP etc...
                std::string cmd;
                cmd += TN_IAC;
                cmd += TN_SB;
                cmd += OPT_STATUS;
...
void cTelnet::setGMCPVariables(const QByteArray &msg)
{
    ...
    if (transcodedMsg.startsWith(QStringLiteral("Client.GUI")))
    {
        ...
        QNetworkReply *reply = mpDownloader->get(request);
        mpProgressDialog = new QProgressDialog(tr("downloading game GUI from server"),
                                               tr("Cancel", "Cancel download of GUI package from Server"), 0, 4000000,
                                               mpHost->mpConsole);
        connect(reply, &QNetworkReply::downloadProgress, this, &cTelnet::setDownloadProgress);
        mpProgressDialog->show();
        return;
...
void cTelnet::atcpComposerSave(QString txt)
{
    ...
    else if (enableGMCP)
    {
        ...
        if (!txt.isEmpty())
        {
            ...
            output += " \"";
            output += encodeAndCookBytes(txt.replace('\\', QLatin1String(R"(\\)"))
...
void cTelnet::postMessage(QString msg)
{
    ...
    while (!messageStack.empty())
    {
        ...
        if (openBraceIndex >= 0 && closeBraceIndex > 0 && closeBraceIndex < hyphenIndex)
        {
            ...
            body.removeFirst();
            if (prefix.contains(
                    tr("ERROR", "Keep the capisalisation, the translated text at 7 letters max so it aligns nicely")) ||
                prefix.contains(QLatin1String("ERROR")))
            {
                mpHost->mpConsole->print(prefix, Qt::red, mpHost->mBgColor); // Bright Red
                mpHost->mpConsole->print(firstLineTail.append('\n'), QColor(255, 255, 50),
                                         mpHost->mBgColor); // Bright Yellow
            ...
            else if (prefix.contains(tr(
                         "LUA", "Keep the capisalisation, the translated text at 7 letters max so it aligns nicely")) ||
                     prefix.contains(QLatin1String("LUA")))
            {
                mpHost->mpConsole->print(prefix, QColor(80, 160, 255), mpHost->mBgColor); // Light blue
                mpHost->mpConsole->print(firstLineTail.append('\n'), QColor(50, 200, 50),
                                         mpHost->mBgColor); // Light green
                for (quint8 _i = 0; _i < body.size(); _i++)
                {
                    QString temp = body.at(_i);
                    temp.replace('\t', QLatin1String("        "));
                ...
                if (!body.empty())
                {
                    mpHost->mpConsole->print(body.join('\n').append('\n'), QColor(200, 50, 50),
                     ...
                     prefix.contains(QLatin1String("WARN")))
            {
                ...
                if (!body.empty())
                {
                    mpHost->mpConsole->print(body.join('\n').append('\n'), QColor(190, 150, 0), mpHost->mBgColor);
                         ...
                         tr("ALERT",
                            "Keep the capisalisation, the translated text at 7 letters max so it aligns nicely")) ||
                     prefix.contains(QLatin1String("ALERT")))
            {
                ...
                {
                    mpHost->mpConsole->print(body.join('\n').append('\n'), QColor(190, 190, 50),
                                             mpHost->mBgColor); // Yellow
                }
            }
            else if (prefix.contains(
                         tr("INFO",
                            "Keep the capisalisation, the translated text at 7 letters max so it aligns nicely")) ||
                     prefix.contains(QLatin1String("INFO")))
            {
                ...
                if (!body.empty())
                {
                    mpHost->mpConsole->print(body.join('\n').append('\n'), QColor(0, 160, 0),
                         ...
                         "OK", "Keep the capisalisation, the translated text at 7 letters max so it aligns nicely")) ||
                     prefix.contains(QLatin1String("OK")))
            {
                mpHost->mpConsole->print(prefix, QColor(0, 160, 0), mpHost->mBgColor); // Light Green
                mpHost->mpConsole->print(firstLineTail.append('\n'), QColor(190, 100, 50),
                                         mpHost->mBgColor); // Orangish
                for (quint8 _i = 0; _i < body.size(); _i++)
                {
                    QString temp = body.at(_i);
                    temp.replace('\t', QLatin1String("        "));
            ...
            { // Unrecognised but still in a "[ something ] -  message..." format
                mpHost->mpConsole->print(prefix, QColor(190, 50, 50),
                                         mpHost->mBgColor); // Foreground red, background bright grey
                mpHost->mpConsole->print(firstLineTail.append('\n'), QColor(50, 50, 50),
                                         mpHost->mBgColor); // Foreground dark grey, background bright grey
                for (quint8 _i = 0; _i < body.size(); _i++)
                {
                    QString temp = body.at(_i);
                    temp.replace('\t', QLatin1String("        "));
        ...
        { // No prefix found
            mpHost->mpConsole->print(body.join('\n').append('\n'), QColor(190, 190, 190),
                                     mpHost->mBgColor); // Foreground bright grey
        }
        ...
        if (recvdGA)
        {
            ...
            if (mUSE_IRE_DRIVER_BUGFIX || mLF_ON_GA)
            {
                cleandata.push_back('\n'); // part of the broken IRE-driver bugfix to make up for broken \n-prepending
                                           // in unsolicited lines, part #2 see line 628
            }
            recvdGA = false;
            gotPrompt(cleandata);
            cleandata = "";
                        ...
                        if ((_ch == OPT_COMPRESS) || (_ch == OPT_COMPRESS2))
                        {
                            ...
                            if ((i > 1) && (i + 2 < datalen))
                            {
                                ...
                                if ((buffer[i - 2] == TN_IAC) && (buffer[i - 1] == TN_SB) &&
                                    (buffer[i + 1] == TN_WILL) && (buffer[i + 2] == TN_SE))
                                {
                                    qDebug() << "MCCP version 1 starting sequence";
                                    _compress = true;
                                }
                                if ((buffer[i - 2] == TN_IAC) && (buffer[i - 1] == TN_SB) &&
                                    (buffer[i + 1] == TN_IAC) && (buffer[i + 2] == TN_SE))
                                {
                                    qDebug() << "MCCP version 2 starting sequence";
                                    _compress = true;
                                }
                                qDebug() << (int)buffer[i - 2] << "," << (int)buffer[i - 1] << "," << (int)buffer[i]
...
std::string cTelnet::encodeAndCookBytes(const std::string &data)
{
    if (mpOutOfBandDataIncomingCodec)
    {
        ...
        return mudlet::replaceString(
            mpOutOfBandDataIncomingCodec->fromUnicode(QString::fromStdString(data)).toStdString(), "\xff", "\xff\xff");
    }
    else
    {
