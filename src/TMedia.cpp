...
void TMedia::playMedia(TMediaData &mediaData)
{
    ...
    if (!mediaData.getMediaFileName().contains('*') && !mediaData.getMediaFileName().contains('?'))
    { // File path wildcards are * and ?
        ...
        {
            return; // A "feature", primarily for MSP, to enable volume 0 (zero) to preload files (already loaded
                    // above).  Processing complete.  Exit!
        }
    }
    ...
    while (itTMediaPlayer.hasNext())
    {
        ...
        if (mediaData.getMediaProtocol() == TMediaData::MediaProtocolGMCP)
        {
            ...
            if (mediaData.getMediaPriority() != TMediaData::MediaPriorityNotSet &&
                pPlayer.getMediaData().getMediaPriority() != TMediaData::MediaPriorityNotSet &&
...
void TMedia::parseGMCP(QString &packageMessage, QString &gmcp)
{
    ...
    if (packageMessage == "Client.Media.Default" || packageMessage == "Client.Media")
    { // Client.Media obsolete
...
QUrl TMedia::parseUrl(TMediaData &mediaData)
{
    ...
    if (mediaData.getMediaProtocol() == TMediaData::MediaProtocolMSP && mediaData.getMediaFileName() == "Off")
    {
        if (mediaData.getMediaUrl().isEmpty())
        { // MSP is !!SOUND(Off) or !!MUSIC(Off)
            mpHost->mpMedia->stopMedia(mediaData);
        }
        else
        { // MSP is !!SOUND(Off U=https://example.com/sounds) or !!MUSIC(Off U=https://example.com/sounds)
    ...
    else if (mediaData.getMediaUrl().isEmpty())
    {
        ...
        else if (mediaData.getMediaProtocol() == TMediaData::MediaProtocolGMCP &&
                 !mpHost->getMediaLocationGMCP().isEmpty())
        {
...
bool TMedia::isValidUrl(QUrl &url)
{
    ...
    if (!url.isValid())
    {
        qWarning() << QStringLiteral("TMedia::validUrl() WARNING - Attempt made to reference an invalid URL: %1 and "
...
bool TMedia::isFileRelative(TMediaData &mediaData)
{
    ...
    {
        qWarning() << QStringLiteral("TMedia::isFileRelative() WARNING - Attempt made to send an absolute path as a "
                                     "media file name: %1.  Only relative paths are permitted.")
...
QStringList TMedia::parseFileNameList(TMediaData &mediaData, QDir &dir)
{
    ...
    if ((mediaData.getMediaFileName().contains('*') || mediaData.getMediaFileName().contains('?')) &&
        mediaData.getMediaFileName().count('*') < 2)
    {
        ...
        {
            fileNameList << QStringLiteral("%1/%2").arg(
                mudlet::getMudletPath(mudlet::profileMediaPath, mpHost->getName()), fileName);
        }
    }
    else
    {
        ...
        fileNameList << QStringLiteral("%1/%2").arg(mudlet::getMudletPath(mudlet::profileMediaPath, mpHost->getName()),
                                                    mediaData.getMediaFileName());
    }
...
QStringList TMedia::getFileNameList(TMediaData &mediaData)
{
    ...
    if (!mediaDir.mkpath(mediaPath))
    {
        qWarning() << QStringLiteral(
                          "TMedia::getFileNameList() WARNING - Attempt made to create a directory failed: %1")
    ...
    {
        QString mediaSubPath =
            QStringLiteral("%1/%2").arg(mudlet::getMudletPath(mudlet::profileMediaPath, mpHost->getName()),
                                        mediaData.getMediaFileName().section('/', 0, -2));
    ...
    // If we did declare a sub directory, but didn't find the file there, we'll try a search in the root directory per
    // the specification.
...
void TMedia::downloadFile(TMediaData &mediaData)
{
    ...
    if (!mediaData.getMediaFileName().isEmpty() && mediaData.getMediaFileName().contains('/'))
    {
        ...
        {
            qWarning() << QStringLiteral(
                              "TMedia::downloadFile() WARNING - Attempt made to create a directory failed: %1")
                              .arg(mudlet::getMudletPath(mudlet::profileMediaPath, mpHost->getName()),
                                   mediaData.getMediaFileName().section('/', 0, -2));
            return;
        }
    ...
    {
        qWarning() << "TMedia::downloadFile() WARNING - Couldn't create cache directory for sound file(s): "
                   << cacheDir;
        return;
    }
    ...
    else
    {
        ...
        connect(getReply, static_cast<void (QNetworkReply::*)(QNetworkReply::NetworkError)>(&QNetworkReply::error),
...
QString TMedia::setupMediaAbsolutePathFileName(TMediaData &mediaData)
{
    ...
    absolutePathFileName = QStringLiteral("%1/%2").arg(
        mudlet::getMudletPath(mudlet::profileMediaPath, mpHost->getName()), mediaData.getMediaFileName());
...
TMediaPlayer TMedia::getMediaPlayer(TMediaData &mediaData)
{
    ...
    { // Find first available inactive QMediaPlayer
        ...
        if (pTestPlayer.getMediaPlayer()->state() != QMediaPlayer::PlayingState &&
            pTestPlayer.getMediaPlayer()->mediaStatus() != QMediaPlayer::LoadingMedia)
        {
            pPlayer = pTestPlayer;
    ...
    { // No available QMediaPlayer, create a new one.
        ...
        { // It should be impossible to ever reach this.
            qWarning() << QStringLiteral(
                "TMedia::getMediaPlayer() WARNING - Unable to create new QMediaPlayer object.");
            return pPlayer;
        }
    }
...
bool TMedia::doesMediaHavePriorityToPlay(TMediaData &mediaData, const QString &absolutePathFileName)
{
    ...
    while (itTMediaPlayer.hasNext())
    { // Find the maximum priority of all playing sounds
        TMediaPlayer pTestPlayer = itTMediaPlayer.next();
    ...
    if (maxMediaPriority >= mediaData.getMediaPriority())
    { // Our media has a lower priority
    ...
    while (itTMediaPlayer.hasNext())
    {
            ...
            pTestPlayer.getMediaPlayer()->mediaStatus() != QMediaPlayer::LoadingMedia)
        {
            if (!mediaData.getMediaKey().isEmpty() && !pTestPlayer.getMediaData().getMediaKey().isEmpty() &&
                mediaData.getMediaKey() == pTestPlayer.getMediaData().getMediaKey())
            { // Does it have the same key?
                if (!pTestPlayer.getMediaPlayer()->media().canonicalUrl().toString().endsWith(absolutePathFileName) ||
                    (!mediaData.getMediaUrl().isEmpty() && !pTestPlayer.getMediaData().getMediaUrl().isEmpty() &&
                     mediaData.getMediaUrl() != pTestPlayer.getMediaData().getMediaUrl()))
                { // Is it a different sound or music than specified?
...
void TMedia::play(TMediaData &mediaData)
{
    ...
    { // Same music is already playing
        if (mediaData.getMediaContinue() == TMediaData::MediaContinueDefault &&
            mediaData.getMediaLoops() == TMediaData::MediaLoopsDefault)
        {
    ...
    if (mediaData.getMediaLoops() == TMediaData::MediaLoopsDefault)
    { // Play once
        ...
        {
            TMedia::matchMediaKeyAndStopMediaVariants(
                mediaData, absolutePathFileName); // If mediaKey matches, check for uniqueness.
        }
    ...
    else
    {
        ...
        if (mediaData.getMediaLoops() == TMediaData::MediaLoopsRepeat)
        { // Repeat indefinitely
            if (fileNameList.size() > 1)
            {
                absolutePathFileName = fileNameList.at(qrand() % fileNameList.size());
        ...
        {
            if (mediaData.getMediaType() == TMediaData::MediaTypeMusic &&
                mediaData.getMediaContinue() == TMediaData::MediaContinueDefault)
            {
                mediaData.setMediaLoops(mediaData.getMediaLoops() -
                                        1); // Subtract the currently playing music from the total
            }

            for (int k = 0; k < mediaData.getMediaLoops(); k++)
            { // Repeat a finite number of times
                ...
                {
                    if (!TMedia::doesMediaHavePriorityToPlay(mediaData, absolutePathFileName))
                    { // Filter out other sound
                        return;
                    }

                    if (mediaData.getMediaProtocol() == TMediaData::MediaProtocolGMCP &&
                        !mediaData.getMediaKey().isEmpty())
                    {
                        TMedia::matchMediaKeyAndStopMediaVariants(
                            mediaData, absolutePathFileName); // If mediaKey matches, check for uniqueness.
                    }
...
int TMedia::parseJSONByMediaPriority(QJsonObject &json)
{
    ...
    if (mediaPriorityJSON != QJsonValue::Undefined && mediaPriorityJSON.isString() &&
        !mediaPriorityJSON.toString().isEmpty())
    {
...
TMediaData::MediaContinue TMedia::parseJSONByMediaContinue(QJsonObject &json)
{
    ...
    if (mediaContinueJSON != QJsonValue::Undefined && mediaContinueJSON.isString() &&
        !mediaContinueJSON.toString().isEmpty())
    {
