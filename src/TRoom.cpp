...
TRoom::TRoom(TRoomDB *pRDB)
    : x(0), y(0), z(0), environment(-1), isLocked(false), min_x(0), min_y(0), max_x(0), max_y(0), mSymbol(QString()),
      highlight(false), highlightColor(QColor(255, 150, 0)), rendered(false), id(0), area(-1), weight(1), north(-1),
      northeast(-1), east(-1), southeast(-1), south(-1), southwest(-1), west(-1), northwest(-1), up(-1), down(-1),
      in(-1), out(-1), mpRoomDB(pRDB), highlightRadius()
{
}
...
void TRoom::setExitStub(int direction, bool status)
{
    if (status)
    {
        ...
        {
            QString error =
                QString("Set exit stub in given direction in RoomID(%1) - there is already an exit there!").arg(id);
...
bool TRoom::setArea(int areaID, bool isToDeferAreaRelatedRecalculations)
{
    ...
    if (!pA)
    {
        ...
        { // Oh, dear THAT didn't work
            QString error =
                qApp->translate("TRoom", "No area created!  Requested area ID=%1. Note: Area IDs must be > 0")
                    .arg(areaID);
            mpRoomDB->mpMap->logError(error);
            return false;
        }
    ...
    {
        QString error =
            qApp->translate("TRoom", "Warning: When setting the Area for Room (Id: %1) it did not have a current area!")
                .arg(id);
        mpRoomDB->mpMap->logError(error);
    }
        ...
        else
        {
            ...
            while (itCustomLineColor.hasNext())
            {
                    ...
                    direction == QLatin1String("IN") || direction == QLatin1String("OUT"))
                {
                    ...
                    {
                        customLinesColor.insert(itCustomLineColor.key().toLower(),
                                                QColor(itCustomLineColor.value().at(0), itCustomLineColor.value().at(1),
                                                       itCustomLineColor.value().at(2)));
                    }
                ...
                else
                {
                    if (itCustomLineColor.value().count() > 2)
                    {
                        customLinesColor.insert(itCustomLineColor.key(),
                                                QColor(itCustomLineColor.value().at(0), itCustomLineColor.value().at(1),
                                                       itCustomLineColor.value().at(2)));
                    }
                }
...
void TRoom::auditExits(const QHash<int, int> roomRemapping)
{
    ...
    auditExit(north, DIR_NORTH, tr("North"), QStringLiteral("n"), exitWeightsCopy, exitStubsCopy, exitLocksCopy,
              doorsCopy, customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy,
              roomRemapping);

    auditExit(northeast, DIR_NORTHEAST, tr("Northeast"), QStringLiteral("ne"), exitWeightsCopy, exitStubsCopy,
              exitLocksCopy, doorsCopy, customLinesCopy, customLinesColorCopy, customLinesStyleCopy,
              customLinesArrowCopy, roomRemapping);

    auditExit(northwest, DIR_NORTHWEST, tr("Northwest"), QStringLiteral("nw"), exitWeightsCopy, exitStubsCopy,
              exitLocksCopy, doorsCopy, customLinesCopy, customLinesColorCopy, customLinesStyleCopy,
              customLinesArrowCopy, roomRemapping);

    auditExit(south, DIR_SOUTH, tr("South"), QStringLiteral("s"), exitWeightsCopy, exitStubsCopy, exitLocksCopy,
              doorsCopy, customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy,
              roomRemapping);

    auditExit(southeast, DIR_SOUTHEAST, tr("Southeast"), QStringLiteral("se"), exitWeightsCopy, exitStubsCopy,
              exitLocksCopy, doorsCopy, customLinesCopy, customLinesColorCopy, customLinesStyleCopy,
              customLinesArrowCopy, roomRemapping);

    auditExit(southwest, DIR_SOUTHWEST, tr("Southwest"), QStringLiteral("sw"), exitWeightsCopy, exitStubsCopy,
              exitLocksCopy, doorsCopy, customLinesCopy, customLinesColorCopy, customLinesStyleCopy,
              customLinesArrowCopy, roomRemapping);

    auditExit(east, DIR_EAST, tr("East"), QStringLiteral("e"), exitWeightsCopy, exitStubsCopy, exitLocksCopy, doorsCopy,
              customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy, roomRemapping);

    auditExit(west, DIR_WEST, tr("West"), QStringLiteral("w"), exitWeightsCopy, exitStubsCopy, exitLocksCopy, doorsCopy,
              customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy, roomRemapping);

    auditExit(up, DIR_UP, tr("Up"), QStringLiteral("up"), exitWeightsCopy, exitStubsCopy, exitLocksCopy, doorsCopy,
              customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy, roomRemapping);

    auditExit(down, DIR_DOWN, tr("Down"), QStringLiteral("down"), exitWeightsCopy, exitStubsCopy, exitLocksCopy,
              doorsCopy, customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy,
              roomRemapping);

    auditExit(in, DIR_IN, tr("In"), QStringLiteral("in"), exitWeightsCopy, exitStubsCopy, exitLocksCopy, doorsCopy,
              customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy, roomRemapping);

    auditExit(out, DIR_OUT, tr("Out"), QStringLiteral("out"), exitWeightsCopy, exitStubsCopy, exitLocksCopy, doorsCopy,
              customLinesCopy, customLinesColorCopy, customLinesStyleCopy, customLinesArrowCopy, roomRemapping);
    ...
    { // Block code to limit scope of iterator
        ...
        while (it.hasNext())
        {
            ...
            if (_cmd.size() <= 0)
            {
                ...
                {
                    QString warnMsg =
                        tr("[ WARN ]  - In room id:%1 removing invalid (special) exit to %2 {with no name!}")
                ...
                mpRoomDB->mpMap->appendRoomErrorMsg(
                    id, tr("[ WARN ]  - Room had an invalid (special) exit to %1 {with no name!} it was removed.")
                            .arg(it.key(), 6, QLatin1Char('0')));
                it.remove();
            }
            else if (!(_cmd.startsWith('1') || _cmd.startsWith('0')))
            {
                ...
                mpRoomDB->mpMap->appendRoomErrorMsg(
                    id,
                    tr(R"([ INFO ]  - Room needed patching {internal fixup} of (special) exit to %1, was: "%2" now: "%3".)")
                        .arg(_nk, 6, QLatin1Char('0'))
                        .arg(_cmd, _nc));
            }
        }
    ...
    if (!doorsCopy.isEmpty())
    {
        ...
        {
            QString infoMsg =
                tr("[ INFO ]  - In room with id: %1 found one or more surplus door items that were removed:\n"
                   "%2.")
                    .arg(id)
    ...
    if (!exitWeightsCopy.isEmpty())
    {
        ...
        {
            QString infoMsg =
                tr("[ INFO ]  - In room with id: %1 found one or more surplus weight items that were removed:\n"
                   "%2.")
                    .arg(id)
                    .arg(extras.join(QStringLiteral(", ")));
            mpRoomDB->mpMap->postMessage(infoMsg);
    ...
    if (!exitLocksCopy.isEmpty())
    {
        ...
        {
            QString infoMsg =
                tr("[ INFO ]  - In room with id: %1 found one or more surplus exit lock items that were removed:\n"
                   "%2.")
                    .arg(id)
                    .arg(extras.join(QStringLiteral(", ")));
            mpRoomDB->mpMap->postMessage(infoMsg);
    ...
    // not valid then remove corresponding entries in other elements
    {
        ...
        if (!extras.isEmpty())
        {
            ...
            {
                QString infoMsg =
                    tr("[ INFO ]  - In room with id: %1 found one or more surplus custom line elements that\n"
                       "were removed: %2.")
                        .arg(id)
                        .arg(extras.join(QStringLiteral(", ")));
                mpRoomDB->mpMap->postMessage(infoMsg);
            ...
            mpRoomDB->mpMap->appendRoomErrorMsg(
                id,
                tr("[ INFO ]  - Room had one or more surplus custom line elements that were removed: %1.")
                    .arg(extras.join(QStringLiteral(", "))),
                true);
        }
    }
...
void TRoom::auditExit(
    int &exitRoomId,                     // Reference to where exit goes to
    ...
    QSet<int> &exitStubsPool, QSet<int> &exitLocksPool, QMap<QString, int> &doorsPool,
    QMap<QString, QList<QPointF>> &customLinesPool, QMap<QString, QColor> &customLinesColorPool,
    QMap<QString, Qt::PenStyle> &customLinesStylePool, QMap<QString, bool> &customLinesArrowPool,
    const QHash<int, int> roomRemapping)
{
    ...
    if (exitRoomId > 0)
    {
        ...
        if (Q_UNLIKELY(!mpRoomDB->getRoom(exitRoomId)))
        {
            ...
            doorsPool.remove(
                exitKey); // we now have a stub exit and that can take a door so clear the door from the check pool
        ...
        else
        {
            ...
            if (exitStubs.contains(dirCode))
            {
                ...
                exitStubsPool.remove(
                    dirCode); // Remove the stub in this direction from check pool as we have handled it
            }
    ...
    else
    {
        ...
        if (exitWeights.contains(exitKey))
        {
            ...
            logMsg.append(
                tr(R"(  It had a weight, this is recorded as user data with key: "%1".)").arg(auditKeyWeight));
            exitWeights.remove(exitKey);
        }
        if (mudlet::self()->showMapAuditErrors())
        {
        ...
        if (customLines.contains(exitKey))
        {
            ...
            mpRoomDB->mpMap->appendRoomErrorMsg(
                id,
                tr("[ WARN ]  - There was a custom exit line associated with the invalid exit but it has not been "
                   "possible to salvage this, it has been lost!"),
                true);
            customLines.remove(exitKey);
        }
        customLinesColor.remove(exitKey);
