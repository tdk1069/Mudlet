...
T2DMap::T2DMap(QWidget *parent)
    : QWidget(parent), mpMap(), xyzoom(20), mRX(), mRY(), mPick(), mTarget(), mStartSpeedWalk(), mRoomBeingMoved(),
      mRoomWidth(), mRoomHeight(), mChosenRoomColor(5), xspan(), yspan(), mMultiSelection(), mMultiRect(), mPopupMenu(),
      mNewMoveAction(), mMapInfoRect(), mFontHeight(20), mShowRoomID(false), gzoom(20), rSize(0.5), eSize(3.0),
      mRoomID(), mAreaID(), mOx(), mOy(), mOz(), mShiftMode(), mShowInfo(true), arealist_combobox(),
      mpCustomLinesDialog(), mCustomLinesRoomFrom(), mCustomLinesRoomTo(), mpCurrentLineStyle(),
      mCurrentLineStyle(Qt::SolidLine), mpCurrentLineColor(), mCurrentLineColor(Qt::red), mpCurrentLineArrow(),
      mCurrentLineArrow(true), mBubbleMode(), mMapperUseAntiAlias(true), mLabelHighlighted(false), mMoveLabel(),
      mCustomLineSelectedRoom(), mCustomLineSelectedExit(), mCustomLineSelectedPoint(-1),
      mMultiSelectionListWidget(this), mSizeLabel(), isCenterViewCall(), mDialogLock(),
      mMultiSelectionHighlightRoomId(0), mIsSelectionSorting(true), mIsSelectionSortByNames(),
      mIsSelectionUsingNames(false), mSymbolFontSize(1), mMaxRoomIdDigits(0)
{
    ...
    mMultiSelectionListWidget.setToolTip(
        tr("<p>Click on a line to select or deselect that room number (with the given name if the "
            ...
            if (!area->zLevels.contains(mOz))
            {
                ...
                if (validRoomFound)
                {
                    ...
                    while (itpRoom.hasNext())
                    {
                        ...
                        QVector2D meanToRoom(static_cast<float>(room->x) - mean_x,
                                             static_cast<float>(room->y) - mean_y);
...
void T2DMap::addSymbolToPixmapCache(const QString key, const bool gridMode)
{
    ...
    symbolPainter.setRenderHints(QPainter::Antialiasing | QPainter::TextAntialiasing | QPainter::SmoothPixmapTransform,
                                 true);
    ...
    if (needToFallback)
    {
        ...
        fontForThisSymbol.setStyleStrategy(
            static_cast<QFont::StyleStrategy>(mpMap->mMapSymbolFont.styleStrategy() & ~(QFont::NoFontMerging)));
    }
 ...
 * 10%.
 */
bool T2DMap::sizeFontToFitTextInRect(QFont &font, const QRectF &boundaryRect, const QString &text,
                                     const quint8 percentageMargin)
{
    QFont _font = font;

    if (percentageMargin > 40)
    {
        qWarning() << "T2DMap::sizeFontToFitTextInRect(...) percentage margin" << percentageMargin
                   << "exceeded recommended maximum (40%) !";
    }
    if (text.isEmpty())
    {
    ...
    do
    {
        ...
        neededRect =
            _painter.boundingRect(testRect, Qt::AlignCenter | Qt::TextSingleLine | Qt::TextIncludeTrailingSpaces, text);
    ...
    if (mpMap->mpRoomDB->getRoom(speedWalkTargetRoomId))
    {
        ...
        else
        {
            mpHost->mpConsole->printSystemMessage(QStringLiteral("%1\n").arg(
                tr("Mapper: Cannot find a path from %1 to %2 using known exits.")
                    .arg(QString::number(speedWalkStartRoomId), QString::number(speedWalkTargetRoomId))));
        }
    }
}
...
// rooms)...
inline void T2DMap::drawRoom(QPainter &painter, QFont &roomVNumFont, QPen &pen, TRoom *pRoom, const bool isGridMode,
                             const bool areRoomIdsLegible, const int speedWalkStartRoomId, const float rx,
                             const float ry, const bool picked)
{
    ...
    {
        roomRectangle = QRectF(rx - (mRoomWidth * rSize) / 2.0, ry - (mRoomHeight * rSize) / 2.0, mRoomWidth * rSize,
                               mRoomHeight * rSize);
    }
    ...
    else
    {
        ...
        if (!(mShowRoomID && areRoomIdsLegible) && !pRoom->mSymbol.isEmpty())
        {
            ...
            if (!pix)
            {
                qWarning("T2DMap::paintEvent() Alert: mSymbolPixmapCache failure, too many items to cache all of them "
                         "for: \"%s\"",
                         pRoom->mSymbol.toUtf8().constData());
            }
            else
            {
                /*
                ...
                painter.drawPixmap(
                    QPoint(qRound(roomRectangle.left() + ((roomRectangle.width() - pix->width()) / 2.0)),
                           qRound(roomRectangle.top() + ((roomRectangle.height() - pix->height()) / 2.0))),
                    *pix);
            }
...
void T2DMap::paintEvent(QPaintEvent *e)
{
    ...
    if (!pPlayerRoom)
    {
        ...
        painter.drawText(0, 0, widgetWidth, widgetHeight, Qt::AlignCenter | Qt::TextWordWrap,
                         tr("No map or no valid position."));
        painter.restore();
        return;
    }
    ...
    if (mShowRoomID)
    {
        ...
        {
            roomTestRect =
                QRectF(0, 0, static_cast<qreal>(mRoomWidth) * rSize, static_cast<qreal>(mRoomHeight) * rSize);
        }
        ...
        roomVNumFont.setStyleStrategy(
            QFont::StyleStrategy(QFont::PreferNoShaping | QFont::PreferAntialias | QFont::PreferOutline));

        isFontBigEnoughToShowRoomVnum = sizeFontToFitTextInRect(
            roomVNumFont, roomTestRect, QStringLiteral("8").repeated(mMaxRoomIdDigits), roomVnumMargin);
    }
    ...
    if (mpMap->mapLabels.contains(mAreaID))
    {
        ...
        while (it.hasNext())
        {
            ...
            {
                mpMap->mapLabels[mAreaID][it.key()].text =
                    tr("no text", "Default text if a label is created in mapper with no text");
            }
            ...
            if (!((0 < labelX || 0 < labelX + labelWidth) &&
                  (widgetWidth > labelX || widgetWidth > labelX + labelWidth)))
            {
                continue;
            }
            if (!((0 < labelY || 0 < labelY + labelHeight) &&
                  (widgetHeight > labelY || widgetHeight > labelY + labelHeight)))
            {
                continue;
            }
            QRectF labelPaintRectangle = QRect(mapLabel.pos.x() * mRoomWidth + mRX,
                                               mapLabel.pos.y() * mRoomHeight * -1 + mRY, labelWidth, labelHeight);
    ...
    while (itRoom.hasNext())
    {
        ...
        else
        {
            ...
            drawRoom(painter, roomVNumFont, pen, room, pArea->gridMode, isFontBigEnoughToShowRoomVnum, playerRoomId, rx,
                     ry, __Pick);
        }
    ...
    {
        drawRoom(painter, roomVNumFont, pen, playerRoom, pArea->gridMode, isFontBigEnoughToShowRoomVnum, playerRoomId,
                 static_cast<float>(playerRoomOnWidgetCoordinates.x()),
                 static_cast<float>(playerRoomOnWidgetCoordinates.y()), __Pick);
        painter.save();
        QPen transparentPen(Qt::transparent);
        QPainterPath myPath;
    ...
    if (mpMap->mapLabels.contains(mAreaID))
    {
        ...
        while (it.hasNext())
        {
            ...
            {
                mpMap->mapLabels[mAreaID][labelID].text =
                    tr("no text", "Default text if a label is created in mapper with no text");
            }
            QPointF labelPosition;
            ...
            QRectF labelPaintRectangle = QRect(label.pos.x() * mRoomWidth + mRX, label.pos.y() * mRoomHeight * -1 + mRY,
    ...
    if (isAreaWidgetValid)
    {
        ...
        { // the area widget is not showing the "default" area
...
void T2DMap::paintAreaExits(QPainter &painter, QPen &pen, QList<int> &exitList, QList<int> &oneWayExits,
                            const TArea *pArea, int zLevel, float exitWidth)
{
    const float widgetWidth = width();
    const float widgetHeight = height();
    ...
    while (itRoom2.hasNext())
    {
        ...
        else
        {
            ...
            if (!((minx > 0.0 || maxx > 0.0) &&
                  (static_cast<float>(widgetWidth) > minx || static_cast<float>(widgetWidth) > maxx)))
            {
                continue;
            }
                    ...
                    if (_id == mCustomLineSelectedRoom && itk.key() == mCustomLineSelectedExit)
                    {
                        ...
                        for (int pk = 2, total = polyLinePoints.size(); pk < total; ++pk)
                        {
                            if (pk == (mCustomLineSelectedPoint + 2))
                            {
                                ...
                                _pen = QPen(QColor(255, 255, 55), _savedPen.width(), Qt::SolidLine, Qt::FlatCap,
                                            _savedPen.joinStyle());
                            }
                            else
                            {
                                _pen = QPen(_savedPen.color(), _savedPen.width(), Qt::SolidLine, Qt::FlatCap,
                                            _savedPen.joinStyle());
                            }
        ...
        for (int direction : qAsConst(room->exitStubs))
        {
            ...
            painter.drawLine(rx + rSize * uDirection.x() / 2.0, ry + rSize * uDirection.y(),
...
void T2DMap::paintMapInfo(const QElapsedTimer &renderTimer, QPainter &painter, const bool showingCurrentArea,
                          QColor &infoColor)
{
    QString infoText;
    int roomID = mRoomID;
            ...
            infoText =
                tr("Area:%1%2 ID:%1%3 x:%1%4%1<‑>%1%5 y:%1%6%1<‑>%1%7 z:%1%8%1<‑>%1%9\n",
                   ...
                   "%4 to %9 are pairs (min <-> max) of extremes for each of x,y and z coordinates")
                    .arg(QChar(160), areaName, QString::number(areaId), QString::number(area->min_x),
                         QString::number(area->max_x), QString::number(area->min_y), QString::number(area->max_y),
                         QString::number(area->min_z), QString::number(area->max_z));
        }
        else
        {
        ...
        case 0:
            infoText.append(tr("Room%1ID:%1%2 Position%1on%1Map: (%3,%4,%5) ‑%1current player location\n",
                               ...
                               "of, and %4-%6 are the x,y and z coordinates for, the current player's room.")
                                .arg(QChar(160), QString::number(roomID), QString::number(_prid->x),
    ...
    do
    {
        ...
        testRect =
            painter.boundingRect(mMapInfoRect.left() + 10, mMapInfoRect.top() + 10, mMapInfoRect.width() - 20,
                                 mMapInfoRect.height() - 20, Qt::TextWordWrap | Qt::AlignLeft | Qt::AlignTop, infoText);
    ...
    painter.setFont(f);
    painter.drawText(mMapInfoRect.left() + 10, mMapInfoRect.top() + 10, mMapInfoRect.width() - 20,
                     mMapInfoRect.height() - 20, Qt::TextWordWrap | Qt::AlignLeft | Qt::AlignTop, infoText);
...
void T2DMap::createLabel(QRectF labelRectangle)
{
    ...
    QPushButton *textButton =
        textOrImageDialog.addButton(tr("Text Label", "2D Mapper create label dialog button"), QMessageBox::ActionRole);
    QPushButton *imageButton =
        textOrImageDialog.addButton(tr("Image Label", "2D Mapper create label dialog button"), QMessageBox::ActionRole);
    ...
    if (textOrImageDialog.clickedButton() == textButton)
    {
        ...
        label.text = text;
        label.bgColor = QColorDialog::getColor(QColor(50, 50, 150, 100), nullptr,
                                               tr("Background color", "2D Mapper create label color dialog title"));
        label.fgColor = QColorDialog::getColor(QColor(255, 255, 50, 255), nullptr,
                                               tr("Foreground color", "2D Mapper create label color dialog title"));
    }
    else if (textOrImageDialog.clickedButton() == imageButton)
    {
        ...
        label.text.clear();
        imagePath =
            QFileDialog::getOpenFileName(nullptr, tr("Select image", "2D Mapper create label file dialog title"));
    }
    else
    {
        return;
    ...
    backgroundOrForegroundDialog.setText(
        tr("Draw label as background or on top of everything?", "2D Mapper create label dialog text"));
    QPushButton *backgroundButton = backgroundOrForegroundDialog.addButton(
        tr("Background", "2D Mapper create label dialog button"), QMessageBox::ActionRole);
    QPushButton *foregroundButton = backgroundOrForegroundDialog.addButton(
        tr("Foreground", "2D Mapper create label dialog button"), QMessageBox::ActionRole);
...
void T2DMap::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::RightButton)
    {
        ...
        if (mCustomLinesRoomFrom > 0)
        {
            ...
            if (room)
            {
                auto customLineUndoLastPoint =
                    new QAction(tr("Undo", "2D Mapper context menu (drawing custom exit line) item"), this);
                customLineUndoLastPoint->setToolTip(
                    tr("Undo last point", "2D Mapper context menu (drawing custom exit line) item tooltip"));
                ...
                auto customLineProperties =
                    new QAction(tr("Properties", "2D Mapper context menu (drawing custom exit line) item name (but not "
                                                 "used as display text as that is set separately)"),
                                this);
                customLineProperties->setText(
                    tr("properties...", "2D Mapper context menu (drawing custom exit line) item display text (has to "
                                        "be entered separately as the ... would get stripped off otherwise)"));
                customLineProperties->setToolTip(tr("Change the properties of this line",
                ...
                auto customLineFinish =
                    new QAction(tr("Finish", "2D Mapper context menu (drawing custom exit line) item"), this);
                customLineFinish->setToolTip(
                    tr("Finish drawing this line", "2D Mapper context menu (drawing custom exit line) item tooltip"));
        ...
        if (!mLabelHighlighted && mCustomLineSelectedRoom == 0)
        {
            ...
            {
                mpCreateRoomAction =
                    new QAction(tr("Create room", "Menu option to create a new room in the mapper"), this);
            ...
            moveRoomXY->setToolTip(
                tr("Move selected group to a given position", "2D Mapper context menu (room) item tooltip"));
            ...
            createLabel->setToolTip(
                tr("Create labels to show text or images", "2D Mapper context menu (room) item tooltip"));
            ...
            else
            {
                ...
                setPlayerLocation->setToolTip(tr("Can only set location when exactly one room is selected",
                                                 "2D Mapper context menu (room) item tooltip (disabled state)"));
            }
        ...
        else
        {
            ...
            //            qDebug("T2DMap::mousePressEvent(): reached else case, mCustomLineSelectedRoom=%i, Exit=%s,
            //            Point=%i",
            ...
            if (mCustomLineSelectedRoom > 0)
            {
                ...
                {
                    auto addPoint =
                        new QAction(tr("Add point", "2D Mapper context menu (custom line editing) item"), this);
                    ...
                    // on the exit direction - and we can now add even to it
                    {
                        ...
                        addPoint->setToolTip(
                            tr("Divide segment by adding a new point mid-way along",
                    ...
                    else
                    {
                        ...
                        addPoint->setToolTip(
                            tr("Select a point first, then add a new point mid-way along the segment towards room",
                               "2D Mapper context menu (custom line editing) item tooltip (disabled state, i.e must do "
                               "the suggested action first)"));
                    }

                    auto removePoint =
                        new QAction(tr("Remove point", "2D Mapper context menu (custom line editing) item"), this);
                    ...
                    if (mCustomLineSelectedPoint > -1)
                    {
                        if (room->customLines.value(mCustomLineSelectedExit).count() > 1)
                        {
                            ...
                            if ((mCustomLineSelectedPoint + 1) <
                                room->customLines.value(mCustomLineSelectedExit).count())
                            {
                                removePoint->setToolTip(
                                    tr("Merge pair of segments by removing this point",
                                       "2D Mapper context menu (custom line editing) item tooltip (enabled state but "
                                       "will be able to be done again on this item)"));
                            }
                            else
                            {
                                removePoint->setToolTip(
                                    tr("Remove last segment by removing this point",
                                       "2D Mapper context menu (custom line editing) item tooltip (enabled state but "
                                       "is the last time this action can be done on this item)"));
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        removePoint->setEnabled(false);
                        removePoint->setToolTip(
                            tr("Select a point first, then remove it",
                               "2D Mapper context menu (custom line editing) item tooltip (disabled state, user will "
                               "need to do something before it can be used)"));
                    }

                    auto lineProperties =
                        new QAction(tr("Properties", "2D Mapper context menu (custom line editing) item name (but not "
                                                     "used as display text as that is set separately)"),
                                    this);
                    ...
                    lineProperties->setText(
                        tr("properties...", "2D Mapper context menu (custom line editing) item display text (has to be "
                                            "entered separately as the ... would get stripped off otherwise"));
                    ...
                    auto deleteLine =
                        new QAction(tr("Delete line", "2D Mapper context menu (custom line editing) item"), this);
    ...
    if (room)
    {
        ...
        if (room->customLines.contains(exit))
        {
            ...
            else
            {
                ...
                {
                    qWarning(
                        R"(T2DMap::slot_customLineProperties() - WARNING: missing command "%s" from custom lines for room id %i)",
                        qPrintable(exit), room->getId());
                }
            }

            mpCurrentLineStyle->setIconSize(QSize(48, 24));
            mpCurrentLineStyle->addItem(QIcon(QPixmap(key_icon_line_solid)), tr("Solid line"),
                                        static_cast<int>(Qt::SolidLine));
            mpCurrentLineStyle->addItem(QIcon(QPixmap(key_icon_line_dot)), tr("Dot line"),
                                        static_cast<int>(Qt::DotLine));
            mpCurrentLineStyle->addItem(QIcon(QPixmap(key_icon_line_dash)), tr("Dash line"),
                                        static_cast<int>(Qt::DashLine));
            mpCurrentLineStyle->addItem(QIcon(QPixmap(key_icon_line_dashDot)), tr("Dash-dot line"),
                                        static_cast<int>(Qt::DashDotLine));
            mpCurrentLineStyle->addItem(QIcon(QPixmap(key_icon_line_dashDotDot)), tr("Dash-dot-dot line"),
...
void T2DMap::slot_customLineAddPoint()
{
    ...
    {
        segment = QLineF(room->customLines.value(mCustomLineSelectedExit).at(mCustomLineSelectedPoint - 1),
                         room->customLines.value(mCustomLineSelectedExit).at(mCustomLineSelectedPoint));
    }
...
void T2DMap::slot_setSymbol()
{
    ...
    if (isAtLeastOneRoom)
    {
        ...
        {
            // No existing symbols
            newSymbol = QInputDialog::getText(
                this, tr("Enter room symbol", "Title for room symbol selection/entry dialog (used in three places)"),
                ...
                QLineEdit::Normal, QString(), &isOk, Qt::Dialog);
        }
        else if (usedSymbols.size() == 1)
        {
            newSymbol = QInputDialog::getText(
                this, tr("Enter room symbol", "Title for room symbol selection/entry dialog (used in three places)"),
                // Intentional comment to separate arguments!
                (mMultiSelectionSet.size() > 1) ? tr("The only used symbol is \"%1\" in one or\n"
                                                ...
                                                : tr("The symbol is \"%1\" in the selected room,\n"
                                                     ...
                                                     "Use line feeds to format text into a reasonable rectangle.")
                                                      .arg(usedSymbols.keys().first()),
                QLineEdit::Normal, usedSymbols.keys().first(), &isOk, Qt::Dialog);
        }
        else
        {
            ...
            newSymbol = QInputDialog::getItem(
                this, // QWidget * parent
                tr("Enter room symbol", "Title for room symbol selection/entry dialog (used in three places)"),
                tr("Choose:\n"
...
void T2DMap::slot_spread()
{
    ...
    bool isOk = false;
    int spread = QInputDialog::getInt(this, tr("Spread out rooms"),
...
void T2DMap::slot_shrink()
{
    ...
    // determined to be null (no change) case, also handle "Cancel" being pressed
    bool isOk = false;
    int spread = QInputDialog::getInt(this, tr("Shrink in rooms"),
...
void T2DMap::slot_setRoomWeight()
{
    ...
    { // Just one room selected
        newWeight = QInputDialog::getInt(this, tr("Enter room weight"),
                                         tr("Enter new roomweight\n"
                                            "(= travel time), minimum\n"
                                            "(and default) is 1:",
                                            // Intentional comment to separate arguments
                                            "Use line feeds to format text into a reasonable rectangle."),
                                         usedWeights.keys().first(), 1, 2147483647, 1, &isOk);
    }
    else
    { // More than one room selected
        ...
        else
        {
            ...
            QString newWeightText = QInputDialog::getItem(
                this,                    // QWidget * parent
...
void T2DMap::slot_loadMap()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(
        this, tr("Load Mudlet map"), mudlet::getMudletPath(mudlet::profileMapsPath, mpMap->mProfileName),
    ...
    if ((mMultiSelection && !mRoomBeingMoved) || mSizeLabel)
    {
        ...
        { // NOT sizing a label
            ...
            while (itSelectedRoom.hasNext())
            {
                ...
                {
                    dr = QRectF(rx - ((mRoomWidth * rSize) / 2.0), ry - ((mRoomHeight * rSize) / 2.0),
                                mRoomWidth * rSize, mRoomHeight * rSize);
                }
...
bool T2DMap::getCenterSelection()
{
    ...
    if (processedRoomCount)
    {
        ...
        while (itRoom.hasNext())
        {
            ...
            QVector3D meanToRoom(static_cast<float>(room->x) - mean_x, static_cast<float>(room->y) - mean_y,
                                 static_cast<float>(room->z) - mean_z);
            if (closestSquareDistance < -0.5)
            {
...
void T2DMap::wheelEvent(QWheelEvent *e)
{
    ...
    // widget:
    QRect selectionListWidgetGlobalRect = QRect(mapToGlobal(mMultiSelectionListWidget.frameRect().topLeft()),
                                                mapToGlobal(mMultiSelectionListWidget.frameRect().bottomRight()));
...
void T2DMap::slot_setCustomLine()
{
    ...
    {
        qWarning(
            R"(T2DMap::slot_setCustomLine() ERROR: failed to find "nw" exit line button or another element of the dialog!)");
        return;
    }
...
void T2DMap::slot_setCustomLine2()
{
    ...
    if (!isFound)
    {
        qWarning() << "T2DMap::slot_setCustomLine2() ERROR - unable to identify exit to use for the button with text:"
                   << pButton->text() << "!)";
        return;
    }
...
void T2DMap::resizeMultiSelectionWidget()
{
    ...
    if (mMultiSelectionListWidget.topLevelItemCount() > 0)
    {
        ...
        { // Have some data rows - and we have forced them to be the same height:
            _newHeight += rowFactor * mMultiSelectionListWidget.topLevelItemCount() *
                          mMultiSelectionListWidget.visualItemRect(rowItem).height();
        }
    }
