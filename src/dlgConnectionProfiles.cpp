...
dlgConnectionProfiles::dlgConnectionProfiles(QWidget *parent)
    : QDialog(parent), mProfileList(QStringList()), offline_button(Q_NULLPTR), connect_button(Q_NULLPTR),
      delete_profile_lineedit(Q_NULLPTR), delete_button(Q_NULLPTR), validName(), validUrl(), validPort(),
      mDefaultGames({"3Kingdoms", "3Scapes", "Aardwolf", "Achaea", "Aetolia", "Avalon.de", "BatMUD", "Clessidra",
                     "Fierymud", "Imperian", "Luminari", "Lusternia", "Materia Magica", "Midnight Sun 2",
                     "Realms of Despair", "Reinos de Leyenda", "StickMUD", "WoTMUD", "ZombieMUD"})
{
    setupUi(this);
    ...
    connect(profiles_tree_widget, &QWidget::customContextMenuRequested, this,
            &dlgConnectionProfiles::slot_profile_menu);
    ...
    auto Welcome_text_template =
        tr("<p><center><big><b>Welcome to Mudlet!</b></big></center></p>"
    ...
    if (mudlet::self()->mShowIconsOnDialogs)
    {
        ...
        // change it anyhow:
        abort->setIcon(
            QIcon::fromTheme(QStringLiteral("dialog-close"), QIcon(QStringLiteral(":/icons/dialog-close.png"))));

        QIcon icon_new(
            QIcon::fromTheme(QStringLiteral("document-new"), QIcon(QStringLiteral(":/icons/document-new.png"))));
        QIcon icon_connect(QIcon::fromTheme(QStringLiteral("dialog-ok-apply"),
                                            QIcon(QStringLiteral(":/icons/preferences-web-browser-cache.png"))));
        ...
        remove_profile_button->setIcon(
            QIcon::fromTheme(QStringLiteral("edit-delete"), QIcon(QStringLiteral(":/icons/edit-delete.png"))));

        copy_profile_toolbutton->setIcon(
        ...
        QTextCursor cursor =
            pWelcome_document->find(QStringLiteral("NEW_PROFILE_ICON"), 0, QTextDocument::FindWholeWords);
        ...
        Q_ASSERT_X(!cursor.isNull(), "dlgConnectionProfiles::dlgConnectionProfiles(...)",
                   "NEW_PROFILE_ICON text marker not found in welcome_message text for when icons are shown on "
                   "dialogue buttons");
        // Remove the marker:
        ...
        Q_ASSERT_X(!cursor.isNull(), "dlgConnectionProfiles::dlgConnectionProfiles(...)",
                   "CONNECT_PROFILE_ICON text marker not found in welcome_message text for when icons are shown on "
                   "dialogue buttons");
        cursor.removeSelectedText();
    ...
    {
        pWelcome_document->setHtml(
            QStringLiteral("<html><head/><body>%1</body></html>").arg(Welcome_text_template.arg(QString(), QString())));
    }
    ...
    {
        character_password_entry->setToolTip(
            tr("Characters password, stored securely in the computer's credential manager"));
    }
    else
    {
        character_password_entry->setToolTip(
            tr("Characters password. Note that the password isn't encrypted in storage"));
    }
    ...
    connect(mud_description_textedit, &QPlainTextEdit::textChanged, this,
            &dlgConnectionProfiles::slot_update_description);
...
QPair<bool, QString> dlgConnectionProfiles::writeProfileData(const QString &profile, const QString &item,
                                                             const QString &what)
{
...
QString dlgConnectionProfiles::getDescription(const QString &hostUrl, const quint16 port,
                                              const QString &profile_name) const
{
    if (hostUrl == QLatin1String("realmsofdespair.com"))
    {
        return QLatin1String(
            "The Realms of Despair is the original SMAUG MUD and is FREE to play. We have an active Roleplaying "
            "community, an active player-killing (deadly) community, and a very active "
            "peaceful community. Players can choose from 13 classes (including a deadly-only class) and 13 races. "
            "Character appearances are customizable on creation and we have a vast "
            "collection of equipment that is level, gender, class, race and alignment specific. We boast well over 150 "
            "original, exclusive areas, with a total of over 20,000 rooms. Mob killing, "
            "or 'running' is one of our most popular activities, with monster difficulties varying from easy "
            "one-player kills to difficult group kills. We have four deadly-only Clans, twelve "
            "peaceful-only Guilds, eight Orders, and fourteen Role-playing Nations that players can join to interact "
            "more closely with other players. We have two mortal councils that actively "
            "work toward helping players: The Symposium hears ideas for changes, and the Newbie Council assists new "
            "players. Our team of Immortals are always willing to answer questions and to "
    ...
    {
        return QLatin1String(
            "Since 1994, ZombieMUD has been on-line and bringing orc-butchering fun to the masses from our home base "
            "in Oulu, Finland. We're a pretty friendly bunch, with players logging in "
            "from all over the globe to test their skill in our medieval role-playing environment. With 15 separate "
            "guilds and 41 races to choose from, as a player the only limitation to your "
            "achievements on the game is your own imagination and will to succeed.");
    }
    else if (hostUrl == QLatin1String("godwars2.org"))
    {
        return QLatin1String(
            "God Wars II is a fast and furious combat mud, designed to test player skill in terms of pre-battle "
            "preparation and on-the-spot reflexes, as well as the ability to adapt quickly to "
            "new situations. Take on the role of a godlike supernatural being in a fight for supremacy.\n\nRoomless "
            "world. Manual combat. Endless possibilities.");
    }
    else if (hostUrl == QLatin1String("3k.org"))
    {
        ...
        {
            return QLatin1String(
                "3Scapes is an alternative dimension to 3Kingdoms, similar in many respects, but unique and twisted in "
                "so many ways.  3Scapes offers a faster pace of play, along with "
                "an assortment "
        ...
        { // port==3000
            return QLatin1String(
                "Simple enough to learn, yet complex enough to challenge you for years, 3Kingdoms is a colossal "
                "adventure through which many years of active and continued development by its "
                "dedicated coding staff.  Based around the mighty town of Pinnacle, three main realms beckon the "
                "player to explore. These kingdoms are known as: Fantasy, a vast medieval realm "
                "full "
                "of orcs, elves, dragons, and a myriad of other creatures; Science, a post-apocalyptic, war-torn world "
                "set in the not-so-distant future; and Chaos, a transient realm where the "
                "enormous realities of Fantasy and Science collide to produce creatures so bizarre that they have yet "
                "to be categorized.  During their exploration of the realms, players have the "
                "opportunity to join any of well over a dozen different guilds, which grant special, unique powers to "
                "the player, furthering their abilities as they explore the vast expanses of "
                "each realm. Add in the comprehensive skill system that 3K offers and you are able to extensively "
    ...
    {
        return QLatin1String(
            "SlothMUD... the ultimate in DIKUMUD! The most active, intricate, exciting FREE MUD of its kind. This text "
            "based multiplayer free online rpg game and is enjoyed continuously by "
            "players worldwide. With over 27,500 uniquely described rooms, 9,300 distinct creatures, 14,200 "
            "characters, and 87,100 pieces of equipment, charms, trinkets and other items, our "
            "online rpg world is absolutely enormous and ready to explore.");
    }
    else if (hostUrl == QLatin1String("game.wotmud.org"))
    {
        return QLatin1String(
            "WoTMUD is the most popular on-line game based on the late Robert Jordan's epic Wheel of Time fantasy "
            "novels.\n"
            "Not only totally FREE to play since it started in 1993 it was officially sanctioned by the Author "
            "himself.\n"
            "Explore a World very like that of Rand al'Thor's; from the Blight in the North down to the Isle of Madmen "
            "far, far south.\n"
            "Wander around in any of the towns from the books such as Caemlyn, Tar Valon or Tear, or start your "
            "adventure in the Two Rivers area, not YET the home of the Dragon Reborn.\n"
            "Will you join one of the Clans working for the triumph of the Light over the creatures and minions of the "
            "Dark One; or will you be one of the returning invaders in the South West, "
            "descendants of Artur Hawkwing's long-thought lost Armies; or just maybe you are skilled enough to be a "
            "hideous Trolloc, creature of the Dark, who like Humans - but only as a source "
            "of sustenance.\n"
            "Very definitely a Player Verses Player (PvP) world but with strong Role Playing (RP) too; nowhere is "
            "totally safe but some parts are much more dangerous than others - once you "
            "enter you may never leave...");
    }
    else if (hostUrl == QStringLiteral("midnightsun2.org"))
    {
        return QLatin1String(
            "Midnight Sun is a medieval fantasy LPmud that has been around since 1991. We are a non-PK, hack-and-slash "
            "game, cooperative rather than competitive in nature, and with a strong "
            "sense of community.");
    }
    else if (hostUrl == QStringLiteral("mudlet.org"))
    {
        return QLatin1String("This isn't a game profile, but a special one for testing Mudlet itself using Busted. You "
                             "can also use it as a starting point to create automated tests for your own profiles!");
    }
    else if (hostUrl == QStringLiteral("luminarimud.com"))
    {
        return QLatin1String("Luminari is a deep, engaging game set in the world of the Luminari - A place where magic "
                             "is entwined with the fabric of reality and the forces of evil and destruction "
                             "are rising from a long slumber to again wreak havoc on the realm.  The gameplay of "
                             "Luminari will be familiar to anyone who has played Dungeons and Dragons, Pathfinder "
                             "or any of the many RPG systems based on the d20 ruleset.");
    }
    else if (hostUrl == QStringLiteral("stickmud.com"))
    {
        return QStringLiteral(
            "StickMUD is a free, medieval fantasy game with a graphical user interface and a depth of features. You "
            "are welcomed into the game world with maps and dashboards to complement your imagination. Newbies escape "
            "quickly into game play with minimal study time. Awaken under the wondrous Mallorn Tree in the center of "
            "Newbie Park and learn by playing. Challenge non-player characters to gain experience, advance level and "
            "maximize your stats. Between battles, sit on the enchanted bench under the Tree to rapidly heal and "
            "reduce wait time. Signs in the park present game features such as races, clans and guilds. Read up on "
            "teasers about the adventures on the path ahead like dragons, castles and sailing. Upon maturing to level "
            "5, join a guild and learn the ways of a Bard, Fighter, Mage, Necromancer, Ninja, Thief, Healer or Priest. "
            "Train skills in both craft and combat aligned with your guild. Participate in frequent game-wide events "
            "to earn points exchanged for gold, experience or skill training. Heroes and villains alike are invited! "
            "Role play is optional and player vs. player combat is allowed in much of the game. StickMUD was born in "
            "Finland in June 1991 and is now hosted in Canada. Our diverse community of players and active game "
            "engineers are ready to welcome new players like you to one of the best text-based multi-player games "
            "ever!");
    }
    else if (hostUrl == QStringLiteral("reinosdeleyenda.es"))
    {
        return QStringLiteral(
            ...
            "Reinos de Leyenda takes place in the ever changing world of Eirea, ravaged by the mischiefs of the gods "
            "after "
            ...
            "This is a PvP MUD which allows the player to set how high are the stakes: the more you risk losing upon "
            "death, the more glory to be earned by your heroism. RP, while "
            "not enforced, is rewarded with non-PvP oriented perks and unique treasure.\n\n"
            "A powerful character customization system allows you to choose your deity –or fully disregard the gods– "
            "and join one of the player-run realms that govern the land "
            "to explore a breathing world, delve into the secrets of the oceans, shape your legacy, craft forgotten "
            "marvels for you –or your allies– and fight for faith, glory or coin.");
        /**
         ...
         *
         * Reinos de Leyenda toma lugar en el siempre cambiante mundo de Eirea, devastado por las intrigas de los dioses
         * tras más de un millar de años de desprecio y cruenta guerra entre sus fanáticos peones mortales.
         *
         * La historia se escribe día a día, tomando en consideración las elecciones de los jugadores para decidir las
         * consecuencias irreversibles de este conflicto imperecedero.
         *
         * Éste es un MUD con PvP que permite al jugador establecer cuánto quiere arriesgar al morir: a más riesgo, más
         * gloria ganará por sus heroicidades. La interpretación (Rol) no está obligada, pero si recompensada con
         * habilidades especiales -no orientadas al combate- y tesoros únicos.
         *
         * El detallado creador del juego te permitirá elegir tu deidad -o renegar completamente de los dioses- y unirte
         * a uno de los reinos que los jugadores se encargan de gobernar para explorar un mundo viviente, sumergirte en
         * los misterios del océano, dar forma a tu legado, forjar maravillas olvidadas para ti -o tus aliados- y luchar
         * por fe, gloria o dinero.
         * -- end translation --
         */
    }
    ...
    {
        return QStringLiteral(
            "Clessidra is the first all italian MUD ever created! On Clessidra you may find only original Areas, all "
            "in italian! Many features make Clessidra one of the best, or the best, MUD in Italy : Advanced travel "
            "mode, fight one to one versus your friend, or enemy, The Arena and its fight, the Mortal Challenge, the "
            "intelligent MOBs and their Quest and fighting style, a random automatic mission assignament and for you "
            "and your friends you must try the advanced Clan system that allows wars and conquest. A mercenary system "
            "to help playing when few players are online, a crafting system to create special object and a graphical "
            "user interface to help newbie and expert players have a better experince. A MUD that evolves with new "
            "challenge, new rules, new skills!");
        /**
         * Original Italian
         * -- begin translation --
         * Clessidra e' il primo MUD completamente in italiano mai creato. Su Clessidra potrete trovare solo aree
         * originali ed in italiano. Molte caratteristiche rendono Clessidra uno dei migliori, se non il migliore, MUD
         * in Italia : Avanzati sistemi di spostamento, sfide uno-contro-uno contro gli amici, o i nemici, L'arena e i
         * combattimenti, Le sfide all'ultimo sangue e i MOB intelligenti con le loro Quest e tecniche di combattimento,
         * un sistema di assegnazione di missioni casuali e un avanzatissimo sistema di Clan che permettera' guerre e
         * conquiste. Disponibilità di mercenari in caso di poca utenza, sistema di produzione/mercato per ottenere
         * esclusivi oggetti, un interfaccia grafica per aiutarti a giocare, sia per i novizi che gli esperti. Un MUD
    ...
    {
        return QStringLiteral(
            "The original vision of FieryMUD was to create a challanging MUD for advanced players. This new reborne "
            "Fiery is a hope to bring back the goals of the past by inflicting certain death on unsuspecting players. "
            "FieryMUD will continue to grow and change through the coming years and those players who seek challenge "
            "and possess imagination will come in search of what the 3D world fails to offer them.");
    }
    else
    {
...
void dlgConnectionProfiles::slot_item_clicked(QListWidgetItem *pItem)
{
    ...
    if (val.isEmpty())
    {
        ...
        {
            val = QStringLiteral(
                "<center><a href='http://www.realmsofdespair.com/'>http://www.realmsofdespair.com</a></center>");
        }
        if (profile_name == QStringLiteral("ZombieMUD"))
        {
        ...
        {
            val = QStringLiteral(
                "<center><a href='http://www.luminarimud.com/'>http://www.luminarimud.com/</a></center>");
        }
        if (profile_name == QStringLiteral("StickMUD"))
        {
    ...
    for (const auto &entry : entries)
    {
        ...
        else if (entry == QLatin1String("autosave.xml"))
        {
            ...
            profile_history->addItem(
                QIcon::fromTheme(QStringLiteral("document-save"), QIcon(QStringLiteral(":/icons/document-save.png"))),
                lastModified.toString(Qt::SystemLocaleLongDate), QVariant(entry));
        }
        else
        {
    ...
    const QString profileLoadedMessage =
        tr("This profile is currently loaded - close it before changing the connection parameters.");
...
// to fit all of the 'Welcome' message
    ...
    if (!deletedDefaultMuds.contains(mudServer))
    {
        ...
        if (!hasCustomIcon(mudServer))
        {
            mi = QIcon(QPixmap(QStringLiteral(":/icons/wotmudicon.png"))
                           .scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation)
                           .copy());
            pM->setIcon(mi);
        }
        else
    ...
    if (!deletedDefaultMuds.contains(mudServer))
    {
        ...
        if (!hasCustomIcon(mudServer))
        {
            mi = QIcon(QPixmap(QStringLiteral(":/icons/midnightsun2.png"))
                           .scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation)
                           .copy());
            pM->setIcon(mi);
        }
    ...
    if (!deletedDefaultMuds.contains(mudServer))
    {
        ...
        if (!hasCustomIcon(mudServer))
        {
            mi = QIcon(QPixmap(QStringLiteral(":/icons/luminari_icon.png"))
                           .scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation)
                           .copy());
            pM->setIcon(mi);
        }
    ...
    if (!deletedDefaultMuds.contains(mudServer))
    {
        ...
        if (!hasCustomIcon(mudServer))
        {
            mi = QIcon(QPixmap(QStringLiteral(":/icons/stickmud_icon.jpg"))
                           .scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation)
                           .copy());
            pM->setIcon(mi);
        }
    ...
    if (!deletedDefaultMuds.contains(mudServer))
    {
        ...
        if (!hasCustomIcon(mudServer))
        {
            mi = QIcon(QPixmap(QStringLiteral(":/icons/clessidra.jpg"))
                           .scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation)
                           .copy());
            pM->setIcon(mi);
        }
    ...
    if (!deletedDefaultMuds.contains(mudServer))
    {
        ...
        {
            profiles_tree_widget->addItem(pM);
            mi = QIcon(QPixmap(QStringLiteral(":/icons/reinosdeleyenda_mud.png"))
                           .scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation)
                           .copy());
            pM->setIcon(mi);
        }
...
bool dlgConnectionProfiles::hasCustomIcon(const QString &profileName) const
{
    return QFileInfo::exists(
        mudlet::getMudletPath(mudlet::profileDataItemPath, profileName, QStringLiteral("profileicon")));
}
...
{
    auto profileIconPath =
        mudlet::getMudletPath(mudlet::profileDataItemPath, profileName, QStringLiteral("profileicon"));
    auto icon =
        QIcon(QPixmap(profileIconPath).scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation).copy());
...
template <typename L> void dlgConnectionProfiles::loadSecuredPassword(const QString &profile, L callback)
{
    ...
    connect(job, &QKeychain::ReadPasswordJob::finished, this, [=](QKeychain::Job *job) {
        if (job->error())
        {
            ...
            {
                qDebug() << "dlgConnectionProfiles::loadSecuredPassword ERROR: couldn't retrieve secure password for"
                         << profile << ", error is:" << error;
            }
        }
...
void dlgConnectionProfiles::slot_profile_menu(QPoint pos)
{
    ...
    if (hasCustomIcon(profileName))
    {
        menu.addAction(
            ...
            this, &dlgConnectionProfiles::slot_reset_custom_icon);
    }
    else
    {
        menu.addAction(QIcon(":/icons/mudlet_main_16px.png"),
                       ...
                       this, &dlgConnectionProfiles::slot_set_custom_icon);
    }
...
void dlgConnectionProfiles::slot_set_custom_icon()
{
    ...
    auto icon =
        QIcon(QPixmap(imageLocation).scaled(QSize(120, 30), Qt::IgnoreAspectRatio, Qt::SmoothTransformation).copy());
...
void dlgConnectionProfiles::slot_password_saved(QKeychain::Job *job)
{
    if (job->error())
    {
        qWarning() << "dlgConnectionProfiles::slot_password_saved ERROR: couldn't save password for"
                   << job->property("profile") << "; error was:" << job->errorString();
    }
...
void dlgConnectionProfiles::slot_password_deleted(QKeychain::Job *job)
{
    if (job->error())
    {
        qWarning() << "dlgConnectionProfiles::slot_password_deleted ERROR: couldn't delete password for"
                   << job->property("profile") << "; error was:" << job->errorString();
    }
...
void dlgConnectionProfiles::slot_copy_profilesettings_only()
{
    ...
    for (QString file : {"url", "port", "password", "login", "description"})
    {
        ...
        auto newFilePath =
            QStringLiteral("%1/%2").arg(mudlet::getMudletPath(mudlet::profileHomePath, profile_name), file);
...
bool dlgConnectionProfiles::copyProfileWidget(QString &profile_name, QString &oldname, QListWidgetItem *&pItem) const
{
    ...
    if (profile_name.at(profile_name.size() - 1).isDigit())
    {
        ...
        do
        {
            profile_name = profile_name.left(profile_name.size() - 1) +
                           QString::number(profile_name.at(profile_name.size() - 1).digitValue() + i++);
...
void dlgConnectionProfiles::saveProfileCopy(const QDir &newProfiledir, const pugi::xml_document &newProfileXml) const
{
    ...
    {
        qDebug() << "dlgConnectionProfiles::copyProfileSettingsOnly ERROR - couldn't create new profile file:"
...
void dlgConnectionProfiles::loadProfile(bool alsoConnect)
{
    ...
    {
        QFile file(
            QStringLiteral("%1%2").arg(folder, profile_history->itemData(profile_history->currentIndex()).toString()));
    ...
    if (needsGenericPackagesInstall)
    {
        ...
        {
            mudlet::self()->packagesToInstallList.append(
                QStringLiteral(":/mudlet-lua/lua/generic-mapper/generic_mapper.xml"));
        }
...
bool dlgConnectionProfiles::validateProfile()
{
    ...
    if (pItem)
    {
        ...
        const QString allowedChars =
            QStringLiteral(". _0123456789-#&aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ");
        ...
        if (pItem->text() != name && mProfileList.contains(name))
        {
            ...
            notificationAreaMessageBox->setText(QStringLiteral("%1\n%2").arg(
                notificationAreaMessageBox->text(), tr("This profile name is already in use.")));
            validName = false;
            valid = false;
        }
        ...
        if (url.indexOf(QRegularExpression(QStringLiteral("^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\.){3}(["
        ...
        else
        {
            ...
            {
                offline_button->setDisabled(true);
                offline_button->setToolTip(tr(
                    "<p>Please set a valid profile name, game server address and the game port before loading.</p>"));
            }
            if (connect_button)
            {
                connect_button->setDisabled(true);
                connect_button->setToolTip(tr("<p>Please set a valid profile name, game server address and the game "
                                              "port before connecting.</p>"));
            }
            return false;
        }
    }
...
void dlgConnectionProfiles::slot_togglePasswordVisibility(const bool showPassword)
{
    ...
    if (mpAction_revealPassword->isChecked())
    {
        ...
        // directly:
        mpAction_revealPassword->setIcon(QIcon::fromTheme(QStringLiteral("password-show-on"),
                                                          QIcon(QStringLiteral(":/icons/password-show-on.png"))));
        mpAction_revealPassword->setToolTip(
            tr("<p>Click to hide the password; it will also hide if another profile is selected.</p>"));
    }
    else
    {
        character_password_entry->setEchoMode(QLineEdit::Password);
        mpAction_revealPassword->setIcon(QIcon::fromTheme(QStringLiteral("password-show-off"),
                                                          QIcon(QStringLiteral(":/icons/password-show-off.png"))));
