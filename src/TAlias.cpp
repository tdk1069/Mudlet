...
TAlias::TAlias(TAlias *parent, Host *pHost)
    : Tree<TAlias>(parent), mpHost(pHost), mNeedsToBeCompiled(true), mModuleMember(false), mModuleMasterFolder(false),
      exportItem(true)
{
}

TAlias::TAlias(const QString &name, Host *pHost)
    : Tree<TAlias>(nullptr), mName(name), mpHost(pHost), mNeedsToBeCompiled(true), mModuleMember(false),
      mModuleMasterFolder(false), exportItem(true)
{
}
    ...
    else
    {
        if (mudlet::debugMode)
        {
            TDebug(QColor(Qt::cyan), QColor(Qt::black))
                    << "Alias name=" << mName << "(" << mRegexCode << ") matched.\n" >>
                0;
        }
    }
...
void TAlias::compileRegex()
{
    ...
    QSharedPointer<pcre> re(
        pcre_compile(mRegexCode.toUtf8().constData(), PCRE_UTF8 | PCRE_UCP, &error, &erroffset, nullptr), pcre_deleter);
    ...
    if (!compileScript())
    {
        if (mudlet::debugMode)
        {
            TDebug(QColor(Qt::white), QColor(Qt::red))
                    << "ERROR: Lua compile error. compiling script of alias:" << mName << "\n" >>
                0;
        }
        mOK_code = false;
    }
